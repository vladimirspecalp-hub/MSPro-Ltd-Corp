...
describe("worktree helpers", () => {
  ...
  it("resolves worktree:make target paths under the user home directory", () => {
    expect(resolveWorktreeMakeTargetPath("mspro-ltd-pr-432")).toBe(
      path.resolve(os.homedir(), "mspro-ltd-pr-432"),
    );
  });

  it("rejects worktree:make names that are not safe directory/branch names", () => {
    expect(() => resolveWorktreeMakeTargetPath("mspro-ltd/pr-432")).toThrow(
  ...
  it("rewrites loopback auth URLs to the new port only", () => {
    ...
    expect(rewriteLocalUrlPort("https://mspro-ltd.example", 3110)).toBe("https://mspro-ltd.example");
  });

  it("builds isolated config and env paths for a worktree", () => {
    const paths = resolveWorktreeLocalPaths({
      cwd: "/tmp/mspro-ltd-feature",
      homeDir: "/tmp/mspro-ltd-worktrees",
    ...
    expect(config.database.embeddedPostgresDataDir).toBe(
      path.resolve("/tmp/mspro-ltd-worktrees", "instances", "feature-worktree-support", "db"),
    );
    ...
    expect(config.storage.localDisk.baseDir).toBe(
      path.resolve("/tmp/mspro-ltd-worktrees", "instances", "feature-worktree-support", "data", "storage"),
    );
    ...
    expect(env.MSPROLTD_HOME).toBe(path.resolve("/tmp/mspro-ltd-worktrees"));
    expect(env.MSPROLTD_INSTANCE_ID).toBe("feature-worktree-support");
    expect(env.MSPROLTD_IN_WORKTREE).toBe("true");
    expect(env.MSPROLTD_WORKTREE_NAME).toBe("feature-worktree-support");
    expect(env.MSPROLTD_WORKTREE_COLOR).toBe("#3abf7a");
    expect(formatShellExports(env)).toContain("export MSPROLTD_INSTANCE_ID='feature-worktree-support'");
  });
  ...
  it("copies the source local_encrypted secrets key into the seeded worktree instance", () => {
    const tempRoot = fs.mkdtempSync(path.join(os.tmpdir(), "mspro-ltd-worktree-secrets-"));
    const originalInlineMasterKey = process.env.MSPROLTD_SECRETS_MASTER_KEY;
    const originalKeyFile = process.env.MSPROLTD_SECRETS_MASTER_KEY_FILE;
    try {
      ...
      if (originalInlineMasterKey === undefined) {
        ...
        process.env.MSPROLTD_SECRETS_MASTER_KEY = originalInlineMasterKey;
      }
      if (originalKeyFile === undefined) {
        delete process.env.MSPROLTD_SECRETS_MASTER_KEY_FILE;
      } else {
        process.env.MSPROLTD_SECRETS_MASTER_KEY_FILE = originalKeyFile;
      }
  ...
  it("writes the source inline secrets master key into the seeded worktree instance", () => {
    ...
    try {
      ...
      copySeededSecretsKey({
        ...
        sourceConfig: buildSourceConfig(),
        sourceEnvEntries: {
          MSPROLTD_SECRETS_MASTER_KEY: "inline-source-master-key",
        },
        targetKeyFilePath: targetKeyPath,
      });
  ...
  it("persists the current agent jwt secret into the worktree env file", async () => {
    const tempRoot = fs.mkdtempSync(path.join(os.tmpdir(), "mspro-ltd-worktree-jwt-"));
    const repoRoot = path.join(tempRoot, "repo");
    const originalCwd = process.cwd();
    const originalJwtSecret = process.env.MSPROLTD_AGENT_JWT_SECRET;

    try {
      fs.mkdirSync(repoRoot, { recursive: true });
      process.env.MSPROLTD_AGENT_JWT_SECRET = "worktree-shared-secret";
      process.chdir(repoRoot);

      await worktreeInitCommand({
        seed: false,
        fromConfig: path.join(tempRoot, "missing", "config.json"),
        home: path.join(tempRoot, ".mspro-ltd-worktrees"),
      });

      const envPath = path.join(repoRoot, ".mspro-ltd", ".env");
      const envContents = fs.readFileSync(envPath, "utf8");
      expect(envContents).toContain("MSPROLTD_AGENT_JWT_SECRET=worktree-shared-secret");
      expect(envContents).toContain("MSPROLTD_WORKTREE_NAME=repo");
      expect(envContents).toMatch(/MSPROLTD_WORKTREE_COLOR=\"#[0-9a-f]{6}\"/);
    } finally {
      process.chdir(originalCwd);
      if (originalJwtSecret === undefined) {
        delete process.env.MSPROLTD_AGENT_JWT_SECRET;
      } else {
        process.env.MSPROLTD_AGENT_JWT_SECRET = originalJwtSecret;
      }
      fs.rmSync(tempRoot, { recursive: true, force: true });
    }
  });
  ...
  it("avoids ports already claimed by sibling worktree instance configs", async () => {
    const tempRoot = fs.mkdtempSync(path.join(os.tmpdir(), "mspro-ltd-worktree-claimed-ports-"));
    const repoRoot = path.join(tempRoot, "repo");
    const homeDir = path.join(tempRoot, ".mspro-ltd-worktrees");
    ...
    try {
      ...
      const config = JSON.parse(fs.readFileSync(path.join(repoRoot, ".mspro-ltd", "config.json"), "utf8"));
  ...
  it("defaults the seed source config to the current repo-local MSProLtd config", () => {
    const tempRoot = fs.mkdtempSync(path.join(os.tmpdir(), "mspro-ltd-worktree-source-config-"));
    const repoRoot = path.join(tempRoot, "repo");
    const localConfigPath = path.join(repoRoot, ".mspro-ltd", "config.json");
    const originalCwd = process.cwd();
    const originalPaperclipConfig = process.env.MSPROLTD_CONFIG;
    ...
    try {
      ...
      if (originalPaperclipConfig === undefined) {
        ...
        process.env.MSPROLTD_CONFIG = originalPaperclipConfig;
      }
      fs.rmSync(tempRoot, { recursive: true, force: true });
    }
  });
  ...
  it("preserves the source config path across worktree:make cwd changes", () => {
    const tempRoot = fs.mkdtempSync(path.join(os.tmpdir(), "mspro-ltd-worktree-source-override-"));
    const sourceConfigPath = path.join(tempRoot, "source", "config.json");
  ...
  it("derives worktree reseed target paths from the adjacent env file", () => {
    const tempRoot = fs.mkdtempSync(path.join(os.tmpdir(), "mspro-ltd-worktree-reseed-target-"));
    const worktreeRoot = path.join(tempRoot, "repo");
    const configPath = path.join(worktreeRoot, ".mspro-ltd", "config.json");
    const envPath = path.join(worktreeRoot, ".mspro-ltd", ".env");
    ...
    try {
      ...
      fs.writeFileSync(
        ...
        [
          ...
          "MSPROLTD_INSTANCE_ID=pap-1132-chat",
        ].join("\n"),
        "utf8",
      );
      expect(
  ...
  it("rejects reseed targets without worktree env metadata", () => {
    const tempRoot = fs.mkdtempSync(path.join(os.tmpdir(), "mspro-ltd-worktree-reseed-target-missing-"));
    const worktreeRoot = path.join(tempRoot, "repo");
    const configPath = path.join(worktreeRoot, ".mspro-ltd", "config.json");
    ...
    try {
      fs.mkdirSync(path.dirname(configPath), { recursive: true });
      fs.writeFileSync(configPath, JSON.stringify(buildSourceConfig()), "utf8");
      fs.writeFileSync(path.join(worktreeRoot, ".mspro-ltd", ".env"), "", "utf8");

      expect(() =>
        resolveWorktreeReseedTargetPaths({
          configPath,
          rootPath: worktreeRoot,
        })).toThrow("does not look like a worktree-local MSProLtd instance");
    } finally {
      fs.rmSync(tempRoot, { recursive: true, force: true });
    }
  });
  ...
  it("reseed preserves the current worktree ports, instance id, and branding", async () => {
    const tempRoot = fs.mkdtempSync(path.join(os.tmpdir(), "mspro-ltd-worktree-reseed-"));
    const repoRoot = path.join(tempRoot, "repo");
    ...
    const sourcePaths = resolveWorktreeLocalPaths({
      cwd: sourceRoot,
      homeDir: path.join(tempRoot, ".mspro-ltd-source"),
      instanceId: "default",
    });
    const originalCwd = process.cwd();
    const originalPaperclipConfig = process.env.MSPROLTD_CONFIG;
    ...
    try {
      ...
      fs.writeFileSync(
        ...
        [
          `MSPROLTD_HOME=${homeDir}`,
          `MSPROLTD_INSTANCE_ID=${currentInstanceId}`,
          "MSPROLTD_WORKTREE_NAME=existing-name",
          "MSPROLTD_WORKTREE_COLOR=\"#112233\"",
        ].join("\n"),
        "utf8",
      );
      ...
      expect(rewrittenEnv).toContain(`MSPROLTD_INSTANCE_ID=${currentInstanceId}`);
      expect(rewrittenEnv).toContain("MSPROLTD_WORKTREE_NAME=existing-name");
      expect(rewrittenEnv).toContain("MSPROLTD_WORKTREE_COLOR=\"#112233\"");
    } finally {
      process.chdir(originalCwd);
      if (originalPaperclipConfig === undefined) {
        delete process.env.MSPROLTD_CONFIG;
  ...
  it("restores the current worktree config and instance data if reseed fails", async () => {
    const tempRoot = fs.mkdtempSync(path.join(os.tmpdir(), "mspro-ltd-worktree-reseed-rollback-"));
    const repoRoot = path.join(tempRoot, "repo");
    const sourceRoot = path.join(tempRoot, "source");
    const homeDir = path.join(tempRoot, ".mspro-ltd-worktrees");
    ...
    try {
      ...
      fs.writeFileSync(currentPaths.configPath, JSON.stringify(currentConfig, null, 2), "utf8");
      fs.writeFileSync(currentPaths.envPath, `MSPROLTD_HOME=${homeDir}\nPAPERCLIP_INSTANCE_ID=${currentInstanceId}\n`, "utf8");
      ...
      expect(restoredEnv).toContain(`MSPROLTD_INSTANCE_ID=${currentInstanceId}`);
  ...
  it("rebinds same-repo workspace paths onto the current worktree root", () => {
    expect(
      rebindWorkspaceCwd({
        sourceRepoRoot: "/Users/example/mspro-ltd",
        targetRepoRoot: "/Users/example/mspro-ltd-pr-432",
        workspaceCwd: "/Users/example/mspro-ltd",
      }),
    ).toBe("/Users/example/mspro-ltd-pr-432");

    expect(
      rebindWorkspaceCwd({
        sourceRepoRoot: "/Users/example/mspro-ltd",
        targetRepoRoot: "/Users/example/mspro-ltd-pr-432",
        workspaceCwd: "/Users/example/mspro-ltd/packages/db",
      }),
    ).toBe("/Users/example/mspro-ltd-pr-432/packages/db");
  });
  ...
  it("copies shared git hooks into a linked worktree git dir", () => {
    const tempRoot = fs.mkdtempSync(path.join(os.tmpdir(), "mspro-ltd-worktree-hooks-"));
    const repoRoot = path.join(tempRoot, "repo");
  ...
  it("creates and initializes a worktree from the top-level worktree:make command", async () => {
    const tempRoot = fs.mkdtempSync(path.join(os.tmpdir(), "mspro-ltd-worktree-make-"));
    const repoRoot = path.join(tempRoot, "repo");
    const fakeHome = path.join(tempRoot, "home");
    const worktreePath = path.join(fakeHome, "mspro-ltd-make-test");
    const originalCwd = process.cwd();
    ...
    try {
      ...
      await worktreeMakeCommand("mspro-ltd-make-test", {
        seed: false,
        home: path.join(tempRoot, ".mspro-ltd-worktrees"),
      });

      expect(fs.existsSync(path.join(worktreePath, ".git"))).toBe(true);
      expect(fs.existsSync(path.join(worktreePath, ".mspro-ltd", "config.json"))).toBe(true);
      expect(fs.existsSync(path.join(worktreePath, ".mspro-ltd", ".env"))).toBe(true);
    } finally {
      process.chdir(originalCwd);
  ...
  it("no-ops on the primary checkout unless --branch is provided", async () => {
    const tempRoot = fs.mkdtempSync(path.join(os.tmpdir(), "mspro-ltd-worktree-repair-primary-"));
    const repoRoot = path.join(tempRoot, "repo");
    const originalCwd = process.cwd();
    ...
    try {
      ...
      expect(fs.existsSync(path.join(repoRoot, ".mspro-ltd", "config.json"))).toBe(false);
      expect(fs.existsSync(path.join(repoRoot, ".mspro-ltd", "worktrees"))).toBe(false);
    } finally {
      process.chdir(originalCwd);
      fs.rmSync(tempRoot, { recursive: true, force: true });
    }
  ...
  it("repairs the current linked worktree when MSProLtd metadata is missing", async () => {
    const tempRoot = fs.mkdtempSync(path.join(os.tmpdir(), "mspro-ltd-worktree-repair-current-"));
    const repoRoot = path.join(tempRoot, "repo");
    const worktreePath = path.join(repoRoot, ".mspro-ltd", "worktrees", "repair-me");
    const sourceConfigPath = path.join(tempRoot, "source-config.json");
    const worktreeHome = path.join(tempRoot, ".mspro-ltd-worktrees");
  ...
  it("creates and repairs a missing branch worktree when --branch is provided", async () => {
    const tempRoot = fs.mkdtempSync(path.join(os.tmpdir(), "mspro-ltd-worktree-repair-branch-"));
    const repoRoot = path.join(tempRoot, "repo");
    const sourceConfigPath = path.join(tempRoot, "source-config.json");
    const worktreeHome = path.join(tempRoot, ".mspro-ltd-worktrees");
    const originalCwd = process.cwd();
    const expectedWorktreePath = path.join(repoRoot, ".mspro-ltd", "worktrees", "feature-repair-me");
    ...
    try {
      ...
      expect(fs.existsSync(path.join(expectedWorktreePath, ".mspro-ltd", "config.json"))).toBe(true);
      expect(fs.existsSync(path.join(expectedWorktreePath, ".mspro-ltd", ".env"))).toBe(true);
    } finally {
      process.chdir(originalCwd);
      fs.rmSync(tempRoot, { recursive: true, force: true });
    }
...
describeEmbeddedPostgres("pauseSeededScheduledRoutines", () => {
  it("pauses only routines with enabled schedule triggers", async () => {
    const tempDb = await startEmbeddedPostgresTestDatabase("mspro-ltd-worktree-routines-");
