...
const ONBOARD_ENV_KEYS = [
  "MSPROLTD_PUBLIC_URL",
  "DATABASE_URL",
  "MSPROLTD_DB_BACKUP_ENABLED",
  "MSPROLTD_DB_BACKUP_INTERVAL_MINUTES",
  "MSPROLTD_DB_BACKUP_RETENTION_DAYS",
  "MSPROLTD_DB_BACKUP_DIR",
  "MSPROLTD_DEPLOYMENT_MODE",
  "MSPROLTD_DEPLOYMENT_EXPOSURE",
  "MSPROLTD_BIND",
  "MSPROLTD_BIND_HOST",
  "MSPROLTD_TAILNET_BIND_HOST",
  "HOST",
  "PORT",
  "SERVE_UI",
  "MSPROLTD_ALLOWED_HOSTNAMES",
  "MSPROLTD_AUTH_BASE_URL_MODE",
  "MSPROLTD_AUTH_PUBLIC_BASE_URL",
  "BETTER_AUTH_URL",
  "BETTER_AUTH_BASE_URL",
  "MSPROLTD_STORAGE_PROVIDER",
  "MSPROLTD_STORAGE_LOCAL_DIR",
  "MSPROLTD_STORAGE_S3_BUCKET",
  "MSPROLTD_STORAGE_S3_REGION",
  "MSPROLTD_STORAGE_S3_ENDPOINT",
  "MSPROLTD_STORAGE_S3_PREFIX",
  "MSPROLTD_STORAGE_S3_FORCE_PATH_STYLE",
  "MSPROLTD_SECRETS_PROVIDER",
  "MSPROLTD_SECRETS_STRICT_MODE",
  "MSPROLTD_SECRETS_MASTER_KEY_FILE",
] as const;
...
function quickstartDefaultsFromEnv(opts?: { preferTrustedLocal?: boolean }): {
  ...
  const publicUrl = preferTrustedLocal
    ? undefined
    : (
      process.env.MSPROLTD_PUBLIC_URL?.trim() ||
      process.env.MSPROLTD_AUTH_PUBLIC_BASE_URL?.trim() ||
  ...
  const deploymentMode = preferTrustedLocal
    ? "local_trusted"
    : (parseEnumFromEnv<DeploymentMode>(process.env.MSPROLTD_DEPLOYMENT_MODE, DEPLOYMENT_MODES) ?? "local_trusted");
  const deploymentExposureFromEnv = parseEnumFromEnv<DeploymentExposure>(
    process.env.MSPROLTD_DEPLOYMENT_EXPOSURE,
    DEPLOYMENT_EXPOSURES,
  );
  const deploymentExposure =
    deploymentMode === "local_trusted" ? "private" : (deploymentExposureFromEnv ?? "private");
  const bindFromEnv = parseEnumFromEnv<BindMode>(process.env.MSPROLTD_BIND, BIND_MODES);
  const customBindHostFromEnv = process.env.MSPROLTD_BIND_HOST?.trim() || undefined;
  ...
  const resolvedBind = resolveRuntimeBind({
    ...
    tailnetBindHost: process.env.MSPROLTD_TAILNET_BIND_HOST?.trim(),
  });
  const authPublicBaseUrl = publicUrl;
  const authBaseUrlModeFromEnv = parseEnumFromEnv<AuthBaseUrlMode>(
    process.env.MSPROLTD_AUTH_BASE_URL_MODE,
    AUTH_BASE_URL_MODES,
  );
  const authBaseUrlMode = authBaseUrlModeFromEnv ?? (authPublicBaseUrl ? "explicit" : "auto");
  const allowedHostnamesFromEnv = process.env.MSPROLTD_ALLOWED_HOSTNAMES
    ? process.env.MSPROLTD_ALLOWED_HOSTNAMES
      .split(",")
  ...
  const storageProvider =
    parseEnumFromEnv<StorageProvider>(process.env.MSPROLTD_STORAGE_PROVIDER, STORAGE_PROVIDERS) ??
    defaultStorage.provider;
  const secretsProvider =
    parseEnumFromEnv<SecretProvider>(process.env.MSPROLTD_SECRETS_PROVIDER, SECRET_PROVIDERS) ??
    defaultSecrets.provider;
  const databaseBackupEnabled = parseBooleanFromEnv(process.env.MSPROLTD_DB_BACKUP_ENABLED) ?? true;
  const databaseBackupIntervalMinutes = Math.max(
    1,
    parseNumberFromEnv(process.env.MSPROLTD_DB_BACKUP_INTERVAL_MINUTES) ?? 60,
  );
  const databaseBackupRetentionDays = Math.max(
    1,
    parseNumberFromEnv(process.env.MSPROLTD_DB_BACKUP_RETENTION_DAYS) ?? 30,
  );
  const defaults: OnboardDefaults = {
    database: {
      ...
      backup: {
        ...
        dir: resolvePathFromEnv(process.env.MSPROLTD_DB_BACKUP_DIR) ?? resolveDefaultBackupDir(instanceId),
      },
    },
    logging: {
      mode: "file",
    ...
    storage: {
      ...
      localDisk: {
        baseDir:
          resolvePathFromEnv(process.env.MSPROLTD_STORAGE_LOCAL_DIR) ?? defaultStorage.localDisk.baseDir,
      },
      s3: {
        bucket: process.env.MSPROLTD_STORAGE_S3_BUCKET ?? defaultStorage.s3.bucket,
        region: process.env.MSPROLTD_STORAGE_S3_REGION ?? defaultStorage.s3.region,
        endpoint: process.env.MSPROLTD_STORAGE_S3_ENDPOINT ?? defaultStorage.s3.endpoint,
        prefix: process.env.MSPROLTD_STORAGE_S3_PREFIX ?? defaultStorage.s3.prefix,
        forcePathStyle:
          parseBooleanFromEnv(process.env.MSPROLTD_STORAGE_S3_FORCE_PATH_STYLE) ??
          defaultStorage.s3.forcePathStyle,
      },
    },
    secrets: {
      provider: secretsProvider,
      strictMode: parseBooleanFromEnv(process.env.MSPROLTD_SECRETS_STRICT_MODE) ?? defaultSecrets.strictMode,
      localEncrypted: {
        keyFilePath:
          resolvePathFromEnv(process.env.MSPROLTD_SECRETS_MASTER_KEY_FILE) ??
  ...
  if (deploymentMode === "local_trusted" && process.env.MSPROLTD_DEPLOYMENT_EXPOSURE !== undefined) {
    ignoredEnvKeys.push({
      key: "MSPROLTD_DEPLOYMENT_EXPOSURE",
      reason: "Ignored because deployment mode local_trusted always forces private exposure",
    });
  }
  if (deploymentMode === "local_trusted" && process.env.MSPROLTD_BIND !== undefined) {
    ignoredEnvKeys.push({
      key: "MSPROLTD_BIND",
      reason: "Ignored because deployment mode local_trusted always uses loopback reachability",
    });
  }
  if (deploymentMode === "local_trusted" && process.env.MSPROLTD_BIND_HOST !== undefined) {
    ignoredEnvKeys.push({
      key: "MSPROLTD_BIND_HOST",
      reason: "Ignored because deployment mode local_trusted always uses loopback reachability",
    });
  }
...
export async function onboard(opts: OnboardOptions): Promise<void> {
  ...
  p.intro(pc.bgCyan(pc.black(" msproltdai onboard ")));
  ...
  if (existingConfig) {
    p.log.message(
      pc.dim("Existing MSProLtd install detected; keeping the current configuration unchanged."),
    );
    p.log.message(pc.dim(`Use ${pc.cyan("msproltdai configure")} if you want to change settings.`));

    const jwtSecret = ensureAgentJwtSecret(configPath);
    const envFilePath = resolveAgentJwtEnvFile(configPath);
    if (jwtSecret.created) {
      p.log.success(`Created ${pc.cyan("MSPROLTD_AGENT_JWT_SECRET")} in ${pc.dim(envFilePath)}`);
    } else if (process.env.MSPROLTD_AGENT_JWT_SECRET?.trim()) {
      p.log.info(`Using existing ${pc.cyan("MSPROLTD_AGENT_JWT_SECRET")} from environment`);
    } else {
      p.log.info(`Using existing ${pc.cyan("MSPROLTD_AGENT_JWT_SECRET")} in ${pc.dim(envFilePath)}`);
    }
    ...
    p.note(
      [
        ...
        "Agent auth: MSPROLTD_AGENT_JWT_SECRET configured",
      ].join("\n"),
      "Configuration ready",
    );
    ...
    p.note(
      [
        `Run: ${pc.cyan("msproltdai run")}`,
        `Reconfigure later: ${pc.cyan("msproltdai configure")}`,
        `Diagnose setup: ${pc.cyan("msproltdai doctor")}`,
      ].join("\n"),
      "Next commands",
    );
    ...
    if (shouldRunNow && !opts.invokedByRun) {
      process.env.MSPROLTD_OPEN_ON_LISTEN = "true";
      const { runCommand } = await import("./run.js");
      await runCommand({ config: configPath, repair: true, yes: true });
      return;
    }

    p.outro("Existing MSProLtd setup is ready.");
    return;
  }
  ...
  if (setupMode === "advanced") {
    ...
    if (database.mode === "postgres" && database.connectionString) {
      ...
      try {
        const { createDb } = await import("@msproltd/db");
        const db = createDb(database.connectionString);
        await db.execute("SELECT 1");
        s.stop("Database connection successful");
      } catch {
        s.stop(pc.yellow("Could not connect to database — you can fix this later with `msproltdai doctor`"));
      }
    }
  ...
  if (server.deploymentMode === "authenticated" && database.mode === "embedded-postgres") {
    p.log.info(
      [
        ...
        `Next: ${pc.cyan("msproltdai run")}`,
        `Then: ${pc.cyan("msproltdai auth bootstrap-ceo")}`,
      ].join("\n"),
    );
  }
