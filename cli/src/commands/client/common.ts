...
export function addCommonClientOptions(command: Command, opts?: { includeCompany?: boolean }): Command {
  command
    .option("-c, --config <path>", "Path to MSProLtd config file")
    .option("-d, --data-dir <path>", "MSProLtd data directory root (isolates state from ~/.mspro-ltd)")
    .option("--context <path>", "Path to CLI context file")
    .option("--profile <name>", "CLI context profile name")
    .option("--api-base <url>", "Base URL for the MSProLtd API")
...
export function resolveCommandContext(
  ...
  const apiBase =
    ...
    process.env.MSPROLTD_API_URL?.trim() ||
    profile.apiBase ||
    inferApiBaseFromConfig(options.config);

  const explicitApiKey =
    options.apiKey?.trim() ||
    process.env.MSPROLTD_API_KEY?.trim() ||
  ...
  const companyId =
    ...
    process.env.MSPROLTD_COMPANY_ID?.trim() ||
    profile.companyId;

  if (opts?.requireCompany && !companyId) {
    throw new Error(
      "Company ID is required. Pass --company-id, set MSPROLTD_COMPANY_ID, or set context profile companyId via `msproltdai context set`.",
    );
  }
...
function inferApiBaseFromConfig(configPath?: string): string {
  const envHost = process.env.MSPROLTD_SERVER_HOST?.trim() || "localhost";
  let port = Number(process.env.MSPROLTD_SERVER_PORT || "");
