...
const DATA_DIR_OPTION_HELP =
  "MSProLtd data directory root (isolates state from ~/.mspro-ltd)";

program
  .name("msproltdai")
  .description("MSProLtd CLI — setup, diagnose, and configure your instance")
  .version(cliVersion);
...
program
  ...
  .option("--run", "Start MSProLtd immediately after saving config", false)
  .action(onboard);

program
  .command("doctor")
  .description("Run diagnostic checks on your MSProLtd setup")
  .option("-c, --config <path>", "Path to config file")
  .option("-d, --data-dir <path>", DATA_DIR_OPTION_HELP)
...
program
  ...
  .option("--filename-prefix <prefix>", "Backup filename prefix", "mspro-ltd")
...
program
  .command("run")
  .description("Bootstrap local setup (onboard + doctor) and run MSProLtd")
  .option("-c, --config <path>", "Path to config file")
  .option("-d, --data-dir <path>", DATA_DIR_OPTION_HELP)
...
heartbeat
  ...
  .option("--api-base <url>", "Base URL for the MSProLtd server API")
