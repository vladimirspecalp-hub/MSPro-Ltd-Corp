...
function formatUserLabel(userId: string | null | undefined, currentUserId?: string | null): string {
  if (!userId || userId === "local-board") return i18n.t("activity.actor.board", { defaultValue: "Board" });
  if (currentUserId && userId === currentUserId) return i18n.t("activity.actor.you", { defaultValue: "You" });
  return i18n.t("activity.actor.user", { defaultValue: "user {{id}}", id: userId.slice(0, 5) });
}
...
export function formatActivityVerb(
  ...
  return i18n.t(`activity.verb.${action}`, {
    defaultValue: ACTIVITY_ROW_VERBS[action] ?? action.replace(/[._]/g, " "),
  });
}
...
export function formatIssueActivityAction(
  ...
  if (
    ...
    const label = i18n.t(`activity.issue_label.${action}`, {
      defaultValue: ISSUE_ACTIVITY_LABELS[action] ?? action,
    });
    return `${label} ${key}${title}`;
  }

  return i18n.t(`activity.issue_label.${action}`, {
    defaultValue: ISSUE_ACTIVITY_LABELS[action] ?? action.replace(/[._]/g, " "),
  });
}
