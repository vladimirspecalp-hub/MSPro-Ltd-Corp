  ...
  return (
    <div className="flex items-start gap-1.5 min-w-0 flex-1">
      ...
      <button
        ...
        onClick={handleCopy}
        title={copied ? t("issue_props.copy_done") : t("issue_props.copy_tooltip")}
      >
        {value}
      </button>
...
export function IssueProperties({
  ...
  const projectName = (id: string | null) => {
    if (!id) return id?.slice(0, 8) ?? t("issue_props.none");
  ...
  const executionParticipantLabel = (value: string) => {
    ...
    if (value.startsWith("user:")) {
      return userLabel(value.slice("user:".length)) ?? t("issue_props.user_fallback");
    }
    return value;
  };
  const reviewerTrigger = reviewerValues.length > 0
    ...
    : <span className="text-sm text-muted-foreground">{t("issue_props.none")}</span>;
  ...
  const currentExecutionLabel = (() => {
    ...
    const stageLabel = issue.executionState.currentStageType === "review"
      ? t("issue_props.stage_review")
      : t("issue_props.stage_approval");
    ...
    if (issue.executionState.status === "changes_requested") {
      return participantLabel
        ? t("issue_props.exec_changes_requested_with", { stage: stageLabel, name: participantLabel })
        : t("issue_props.exec_changes_requested_no_actor", { stage: stageLabel });
    }
    return participantLabel
      ? t("issue_props.exec_pending_with", { stage: stageLabel, name: participantLabel })
      : t("issue_props.exec_pending_no_actor", { stage: stageLabel });
  })();
      ...
      <div className="mt-2 border-t border-border pt-2 space-y-1">
        <div className="flex items-center gap-1">
          ...
          <input
            ...
            placeholder={t("issue_props.new_label_placeholder")}
            value={newLabelName}
        ...
        >
          <Plus className="h-3 w-3" />
          {createLabel.isPending ? t("issue_props.creating_label") : t("issue_props.create_label")}
        </button>
      </div>
    </>
  );
    ...
    <>
      ...
      <div className="max-h-48 overflow-y-auto overscroll-contain">
        ...
        {issue.createdByUserId && issue.createdByUserId !== currentUserId && (
          ...
          >
            <User className="h-3 w-3 shrink-0 text-muted-foreground" />
            {creatorUserLabel ? t("issue_props.assign_to_user", { name: creatorUserLabel }) : t("issue_props.assign_to_requester")}
          </button>
        )}
        {sortedAgents
          .filter((a) => {
  ...
  return (
    <div className="space-y-4">
      <div className="space-y-1">
        <PropertyRow label={t("issue_props.label_status")}>
          <StatusIcon
            status={issue.status}
        ...
        <PropertyRow label={t("issue_props.label_priority")}>
          <PriorityIcon
        ...
        <PropertyPicker
          inline={inline}
          label={t("issue_props.label_labels")}
          open={labelsOpen}
        ...
        <PropertyPicker
          inline={inline}
          label={t("issue_props.label_assignee")}
          open={assigneeOpen}
        ...
        <PropertyPicker
          inline={inline}
          label={t("issue_props.label_project")}
          open={projectOpen}
        ...
        <PropertyPicker
          inline={inline}
          label={t("issue_props.label_parent")}
          open={parentOpen}
          onOpenChange={(open) => {
            setParentOpen(open);
        ...
        <PropertyPicker
          inline={inline}
          label={t("issue_props.label_blocked_by")}
          open={blockedByOpen}
          onOpenChange={(open) => {
        ...
        <PropertyRow label={t("issue_props.label_blocking")}>
        ...
        <PropertyRow label={t("issue_props.label_sub_issues")}>
        ...
        <PropertyPicker
          inline={inline}
          label={t("issue_props.label_reviewers")}
          open={reviewersOpen}
        ...
        <PropertyPicker
          inline={inline}
          label={t("issue_props.label_approvers")}
          open={approversOpen}
        ...
        {currentExecutionLabel && (
          <PropertyRow label={t("issue_props.label_execution")}>
            <span className="text-sm">{currentExecutionLabel}</span>
          </PropertyRow>
        )}

        {issue.requestDepth > 0 && (
          <PropertyRow label={t("issue_props.label_depth")}>
        ...
        <>
          ...
          <div className="space-y-1">
            {issue.executionWorkspaceId && (
              <PropertyRow label={t("issue_props.label_workspace")}>
                <Link
                  to={`/execution-workspaces/${issue.executionWorkspaceId}`}
                  className="text-sm text-primary hover:underline inline-flex items-center gap-1"
                >
                  {t("issue_props.view_workspace")}
                  <ExternalLink className="h-3 w-3" />
                </Link>
              </PropertyRow>
            )}
            {issue.currentExecutionWorkspace?.branchName && (
              <PropertyRow label={t("issue_props.label_branch")}>
                <TruncatedCopyable
            ...
            {issue.currentExecutionWorkspace?.cwd && (
              <PropertyRow label={t("issue_props.label_folder")}>
                <TruncatedCopyable
      ...
      <div className="space-y-1">
        {(issue.createdByAgentId || issue.createdByUserId) && (
          <PropertyRow label={t("issue_props.label_created_by")}>
            {issue.createdByAgentId ? (
              ...
              <>
                <User className="h-3.5 w-3.5 text-muted-foreground" />
                <span className="text-sm">{creatorUserLabel ?? t("issue_props.user_fallback")}</span>
              </>
            )}
          </PropertyRow>
        )}
        {issue.startedAt && (
          <PropertyRow label={t("issue_props.label_started")}>
            <span className="text-sm">{formatDate(issue.startedAt)}</span>
          </PropertyRow>
        )}
        {issue.completedAt && (
          <PropertyRow label={t("issue_props.label_completed")}>
            <span className="text-sm">{formatDate(issue.completedAt)}</span>
          </PropertyRow>
        )}
        <PropertyRow label={t("issue_props.label_created")}>
          <span className="text-sm">{formatDate(issue.createdAt)}</span>
        </PropertyRow>
        <PropertyRow label={t("issue_props.label_updated")}>
