...
const ISSUE_THINKING_EFFORT_OPTIONS = {
  claude_local: [
    { value: "", labelKey: "issue_dialog.thinking_default" },
    { value: "low", labelKey: "issue_dialog.thinking_low" },
    { value: "medium", labelKey: "issue_dialog.thinking_medium" },
    { value: "high", labelKey: "issue_dialog.thinking_high" },
  ],
  codex_local: [
    { value: "", labelKey: "issue_dialog.thinking_default" },
    { value: "minimal", labelKey: "issue_dialog.thinking_minimal" },
    { value: "low", labelKey: "issue_dialog.thinking_low" },
    { value: "medium", labelKey: "issue_dialog.thinking_medium" },
    { value: "high", labelKey: "issue_dialog.thinking_high" },
    { value: "xhigh", labelKey: "X-High" },
  ],
  opencode_local: [
    { value: "", labelKey: "issue_dialog.thinking_default" },
    { value: "minimal", labelKey: "issue_dialog.thinking_minimal" },
    ...
    { value: "high", labelKey: "issue_dialog.thinking_high" },
    { value: "xhigh", labelKey: "X-High" },
    { value: "max", labelKey: "issue_dialog.thinking_max" },
  ],
} as const;
...
const statuses = [
  { value: "backlog", labelKey: "issue.status_backlog", color: issueStatusText.backlog ?? issueStatusTextDefault },
  { value: "todo", labelKey: "issue.status_todo", color: issueStatusText.todo ?? issueStatusTextDefault },
  { value: "in_progress", labelKey: "issue.status_in_progress", color: issueStatusText.in_progress ?? issueStatusTextDefault },
  { value: "in_review", labelKey: "issue.status_in_review", color: issueStatusText.in_review ?? issueStatusTextDefault },
  { value: "done", labelKey: "issue.status_done", color: issueStatusText.done ?? issueStatusTextDefault },
];

const priorities = [
  { value: "critical", labelKey: "issue.priority_critical", icon: AlertTriangle, color: priorityColor.critical ?? priorityColorDefault },
  { value: "high", labelKey: "issue.priority_high", icon: ArrowUp, color: priorityColor.high ?? priorityColorDefault },
  { value: "medium", labelKey: "issue.priority_medium", icon: Minus, color: priorityColor.medium ?? priorityColorDefault },
  { value: "low", labelKey: "issue.priority_low", icon: ArrowDown, color: priorityColor.low ?? priorityColorDefault },
];

const EXECUTION_WORKSPACE_MODES = [
  { value: "shared_workspace", labelKey: "issue_dialog.execution_workspace_mode_shared" },
  { value: "isolated_workspace", labelKey: "issue_dialog.execution_workspace_mode_isolated" },
  { value: "reuse_existing", labelKey: "issue_dialog.execution_workspace_mode_reuse" },
] as const;
...
export function NewIssueDialog() {
  ...
  const assigneeOptionsTitle =
    assigneeAdapterType === "claude_local"
      ? t("issue_dialog.agent_options_claude")
      : assigneeAdapterType === "codex_local"
        ? t("issue_dialog.agent_options_codex")
        : assigneeAdapterType === "opencode_local"
          ? t("issue_dialog.agent_options_opencode")
        : t("issue_dialog.agent_options_default");
  ...
  const createIssueErrorMessage =
    createIssue.error instanceof Error ? createIssue.error.message : t("issue_dialog.create_failed_default");
      ...
      >
        ...
        <div className="flex items-center justify-between px-4 py-2.5 border-b border-border shrink-0">
          <div className="flex items-center gap-2 text-sm text-muted-foreground">
            ...
            <span>{isSubIssueMode ? t("issue_dialog.title_new_sub") : t("issue_dialog.title_new")}</span>
          </div>
        ...
        <div className="min-h-0 flex-1 overflow-y-auto overscroll-contain">
          ...
          <div className="px-4 pt-4 pb-2">
            ...
            placeholder={t("issue_dialog.title_placeholder")}
            rows={1}
            value={title}
            onChange={(e) => {
          ...
          <div className="px-4 pb-2">
            <div className="overflow-x-auto overscroll-x-contain">
              ...
              <span className="w-6 shrink-0 text-center">{t("issue_dialog.for")}</span>
              <InlineEntitySelector
                ref={assigneeSelectorRef}
                value={assigneeValue}
                options={assigneeOptions}
                placeholder={t("issue_dialog.assignee")}
                disablePortal
                noneLabel={t("issue_dialog.assignee_none")}
                searchPlaceholder={t("issue_dialog.assignee_search")}
                emptyMessage={t("issue_dialog.assignee_empty")}
                onChange={(value) => {
                ...
                renderTriggerValue={(option) =>
                  option ? (
                    ...
                    <span className="text-muted-foreground">{t("issue_dialog.assignee")}</span>
                  )
                }
                renderOption={(option) => {
              ...
              />
              <span>{t("issue_dialog.in")}</span>
              <InlineEntitySelector
                ref={projectSelectorRef}
                value={projectId}
                options={projectOptions}
                placeholder={t("issue_dialog.project")}
                disablePortal
                noneLabel={t("issue_dialog.project_none")}
                searchPlaceholder={t("issue_dialog.project_search")}
                emptyMessage={t("issue_dialog.project_empty")}
                ...
                renderTriggerValue={(option) =>
                  option && currentProject ? (
                    ...
                    <span className="text-muted-foreground">{t("issue_dialog.project")}</span>
                  )
                }
                renderOption={(option) => {
                  if (!option.id) return <span className="truncate">{option.label}</span>;
              ...
              <Popover open={participantMenuOpen} onOpenChange={setParticipantMenuOpen}>
                <PopoverTrigger asChild>
                  <button
                    ...
                    title={t("issue_dialog.add_participant_title")}
                  >
            ...
            {showReviewerRow && (
              <div className="flex items-center gap-2 text-sm text-muted-foreground mt-1">
                ...
                value={reviewerValue}
                options={assigneeOptions}
                placeholder={t("issue_dialog.reviewer")}
                disablePortal
                noneLabel={t("issue_dialog.reviewer_none")}
                searchPlaceholder={t("issue_dialog.reviewer_search")}
                emptyMessage={t("issue_dialog.reviewer_empty")}
                ...
                renderTriggerValue={(option) =>
                  option ? (
                    ...
                    <span className="text-muted-foreground">{t("issue_dialog.reviewer")}</span>
                  )
                }
                renderOption={(option) => {
                  if (!option.id) return <span className="truncate">{option.label}</span>;
            ...
            {showApproverRow && (
              <div className="flex items-center gap-2 text-sm text-muted-foreground mt-1">
                ...
                value={approverValue}
                options={assigneeOptions}
                placeholder={t("issue_dialog.approver")}
                disablePortal
                noneLabel={t("issue_dialog.approver_none")}
                searchPlaceholder={t("issue_dialog.approver_search")}
                emptyMessage={t("issue_dialog.approver_empty")}
                ...
                renderTriggerValue={(option) =>
                  option ? (
                    ...
                    <span className="text-muted-foreground">{t("issue_dialog.approver")}</span>
                  )
                }
                renderOption={(option) => {
                  if (!option.id) return <span className="truncate">{option.label}</span>;
          ...
          {isSubIssueMode ? (
            ...
            <div className="max-w-full rounded-md border border-border bg-muted/30 px-2.5 py-1.5 text-xs text-muted-foreground">
              <div className="flex items-center gap-1.5">
                ...
                <span className="shrink-0">{t("issue_dialog.sub_issue_of")}</span>
          ...
          {currentProject && currentProjectSupportsExecutionWorkspace && (
            ...
            <div className="space-y-1.5">
              <div className="text-xs font-medium">{t("issue_dialog.execution_workspace_label")}</div>
              <div className="text-[11px] text-muted-foreground">
                {t("issue_dialog.execution_workspace_help")}
              </div>
              <select
              ...
              {executionWorkspaceMode === "reuse_existing" && (
                ...
                >
                  <option value="">{t("issue_dialog.execution_workspace_choose_existing")}</option>
              ...
              {executionWorkspaceMode === "reuse_existing" && selectedReusableExecutionWorkspace && (
                <div className="text-[11px] text-muted-foreground">
                  {t("issue_dialog.execution_workspace_reusing", {
                    name: selectedReusableExecutionWorkspace.name,
                    source: selectedReusableExecutionWorkspace.branchName ?? selectedReusableExecutionWorkspace.cwd ?? t("issue_dialog.execution_workspace_existing_fallback"),
                  })}
                </div>
              )}
              {showParentWorkspaceWarning ? (
                <div className="rounded-md border border-amber-300/60 bg-amber-50 px-2 py-1.5 text-[11px] text-amber-900 dark:border-amber-800/70 dark:bg-amber-950/30 dark:text-amber-100">
                  {t("issue_dialog.execution_workspace_warning", { suffix: parentExecutionWorkspaceLabel ? ` (${parentExecutionWorkspaceLabel})` : "" })}
                </div>
              ) : null}
            </div>
            </div>
          ...
          {supportsAssigneeOverrides && (
            ...
            {assigneeOptionsOpen && (
              <div className="mt-2 rounded-md border border-border p-3 bg-muted/20 space-y-3">
                <div className="space-y-1.5">
                  <div className="text-xs text-muted-foreground">{t("issue_dialog.model")}</div>
                  <InlineEntitySelector
                    value={assigneeModelOverride}
                    options={modelOverrideOptions}
                    placeholder={t("issue_dialog.model_default")}
                    disablePortal
                    noneLabel={t("issue_dialog.model_default")}
                    searchPlaceholder={t("issue_dialog.model_search")}
                    emptyMessage={t("issue_dialog.model_empty")}
                    onChange={setAssigneeModelOverride}
                  />
                </div>
                <div className="space-y-1.5">
                  <div className="text-xs text-muted-foreground">{t("issue_dialog.thinking_effort")}</div>
                  <div className="flex items-center gap-1.5 flex-wrap">
                    {thinkingEffortOptions.map((option) => (
                      ...
                      >
                        {option.labelKey.startsWith("issue_dialog.") ? t(option.labelKey) : option.labelKey}
                      </button>
                    ))}
                  </div>
                </div>
                {assigneeAdapterType === "claude_local" && (
                  <div className="flex items-center justify-between rounded-md border border-border px-2 py-1.5">
                    <div className="text-xs text-muted-foreground">{t("issue_dialog.enable_chrome")}</div>
                    <ToggleSwitch
          ...
          >
            ...
            >
              <MarkdownEditor
                ...
                placeholder={t("issue_dialog.description_placeholder")}
                bordered={false}
            ...
            {stagedFiles.length > 0 ? (
              ...
              {stagedDocuments.length > 0 ? (
                <div className="space-y-2">
                  <div className="text-xs font-medium text-muted-foreground">{t("issue_dialog.documents")}</div>
                  <div className="space-y-2">
                    {stagedDocuments.map((file) => (
                      <div key={file.id} className="flex items-start justify-between gap-3 rounded-md border border-border/70 px-3 py-2">
                        ...
                        <Button
                          ...
                          disabled={createIssue.isPending}
                          title={t("issue_dialog.remove_document")}
                        >
              ...
              {stagedAttachments.length > 0 ? (
                <div className="space-y-2">
                  <div className="text-xs font-medium text-muted-foreground">{t("issue_dialog.attachments")}</div>
                  <div className="space-y-2">
                    {stagedAttachments.map((file) => (
                      <div key={file.id} className="flex items-start justify-between gap-3 rounded-md border border-border/70 px-3 py-2">
                        ...
                        <Button
                          ...
                          onClick={() => removeStagedFile(file.id)}
                          disabled={createIssue.isPending}
                          title={t("issue_dialog.remove_attachment")}
                        >
                          <X className="h-3.5 w-3.5" />
                        </Button>
                      </div>
              ...
              <button className="inline-flex items-center gap-1.5 rounded-md border border-border px-2 py-1 text-xs hover:bg-accent/50 transition-colors">
                {currentPriority ? (
                  ...
                  <>
                    ...
                    {t("issue_dialog.priority_placeholder")}
                  </>
                )}
              </button>
            </PopoverTrigger>
          ...
          <Popover open={moreOpen} onOpenChange={setMoreOpen}>
            ...
            <PopoverContent className="w-44 p-1" align="start">
              <button className="flex items-center gap-2 w-full px-2 py-1.5 text-xs rounded hover:bg-accent/50 text-muted-foreground">
                ...
                {t("issue_dialog.start_date")}
              </button>
              <button className="flex items-center gap-2 w-full px-2 py-1.5 text-xs rounded hover:bg-accent/50 text-muted-foreground">
                <Calendar className="h-3 w-3" />
                {t("issue_dialog.due_date")}
              </button>
            </PopoverContent>
          </Popover>
        </div>
          ...
          <div className="flex items-center gap-3">
            ...
            >
              <span className="inline-flex items-center justify-center gap-1.5">
                ...
                <span>{createIssue.isPending ? t("issue_dialog.submit_creating") : isSubIssueMode ? t("issue_dialog.submit_create_sub") : t("issue_dialog.submit_create")}</span>
              </span>
            </Button>
          </div>
        </div>
