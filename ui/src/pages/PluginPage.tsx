...
export function PluginPage() {
  ...
  useEffect(() => {
    if (pageSlot) {
      setBreadcrumbs([
        { label: t("plugin_page.breadcrumb_plugins"), href: "/instance/settings/plugins" },
  ...
  if (!resolvedCompanyId) {
    ...
    return (
      <div className="space-y-4">
        <p className="text-sm text-muted-foreground">{t("plugin_page.select_company")}</p>
      </div>
    );
  }

  if (!contributions) {
    return <div className="text-sm text-muted-foreground">{t("plugin_page.loading")}</div>;
  }
    ...
    if (duplicateMatches.length > 1) {
      return (
        <div
          ...
          dangerouslySetInnerHTML={{ __html: t("plugin_page.duplicate_route_html", { path: pluginRoutePath }) }}
        />
      );
    }
  }
