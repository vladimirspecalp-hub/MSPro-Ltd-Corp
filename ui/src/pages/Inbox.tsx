...
export function FailedRunInboxRow({
  ...
  const displayError = runFailureMessage(run, t("inbox.run_exited_error"));
          ...
          <span className="hidden sm:inline-flex h-4 w-4 shrink-0 items-center justify-center self-center">
            {showUnreadDot ? (
              <button
                ...
                aria-label={t("inbox.mark_as_read_aria")}
              >
                <span className={cn(
          ...
          <span className="min-w-0 flex-1">
            <span className="line-clamp-2 text-sm font-medium sm:truncate sm:line-clamp-none">
              {issue ? (
                ...
                <>{linkedAgentName ? t("inbox.failed_run_with_agent", { name: linkedAgentName }) : t("inbox.failed_run")}</>
              )}
            </span>
        ...
        >
          ...
          <span className="min-w-0 flex-1">
            ...
            <span className="mt-1 flex flex-wrap items-center gap-x-2 gap-y-1 text-xs text-muted-foreground">
              ...
              {requesterName ? <span>{t("inbox.requested_by", { name: requesterName })}</span> : null}
  ...
  const label =
    joinRequest.requestType === "human"
      ...
      : joinRequest.agentName
        ? t("inbox.agent_join_request_named", { name: joinRequest.agentName })
        : t("inbox.agent_join_request");
  const showUnreadSlot = unreadState !== null;
  const showUnreadDot = unreadState === "visible" || unreadState === "fading";
      ...
      <div className="flex items-start gap-2 sm:items-center">
        ...
        <div className="flex min-w-0 flex-1 items-start gap-2">
          ...
          <span className="min-w-0 flex-1">
            ...
            <span className="mt-1 flex flex-wrap items-center gap-x-2 gap-y-1 text-xs text-muted-foreground">
              <span>{t("inbox.join_requested_meta", { time: timeAgo(joinRequest.createdAt), ip: joinRequest.requestIp })}</span>
              {joinRequest.adapterType && <span>{t("inbox.join_adapter_meta", { adapter: joinRequest.adapterType })}</span>}
            </span>
          </span>
        </div>
        <div className="hidden shrink-0 items-center gap-2 sm:flex">
...
export function Inbox() {
  ...
  const creatorOptions = useMemo<CreatorOption[]>(() => {
    ...
    if (currentUserId) {
      options.set(`user:${currentUserId}`, {
        ...
        label: currentUserId === "local-board" ? t("issue_thread.board") : t("issues.me"),
        kind: "user",
  ...
  const filteredWorkItems = useMemo(() => {
    ...
    return workItemsToRender.filter((item) => {
      ...
      if (item.kind === "failed_run") {
        ...
        const msg = runFailureMessage(run, t("inbox.run_exited_error"));
  ...
  const approveMutation = useMutation({
    ...
    onError: (err) => {
      setActionError(err instanceof Error ? err.message : t("inbox.approve_failed"));
    },
  });
  ...
  const rejectMutation = useMutation({
    ...
    onError: (err) => {
      setActionError(err instanceof Error ? err.message : t("inbox.reject_failed"));
    },
  });
  ...
  const approveJoinMutation = useMutation({
    ...
    onError: (err) => {
      setActionError(err instanceof Error ? err.message : t("inbox.approve_join_failed"));
    },
  });
  ...
  const rejectJoinMutation = useMutation({
    ...
    onError: (err) => {
      setActionError(err instanceof Error ? err.message : t("inbox.reject_join_failed"));
    },
  });
  ...
  const retryRunMutation = useMutation({
    mutationFn: async (run: HeartbeatRun) => {
      ...
      if (!("id" in result)) {
        throw new Error(result.message ?? t("inbox.retry_skipped"));
      }
  ...
  const archiveIssueMutation = useMutation({
    ...
    onError: (err, id, context) => {
      setActionError(err instanceof Error ? err.message : t("inbox.archive_failed"));
  ...
  const unarchiveIssueMutation = useMutation({
    ...
    onError: (err) => {
      setActionError(err instanceof Error ? err.message : t("inbox.undo_archive_failed"));
    },
    onSuccess: (_data, id) => {
    ...
    <div className="space-y-6">
      <div className="space-y-2">
        ...
        <Tabs value={tab} onValueChange={(value) => navigate(`/inbox/${value}`)}>
          <PageTabBar
            items={[
              {
                value: "mine",
                label: t("inbox.tab_mine"),
              },
              {
                value: "recent",
                label: t("inbox.tab_recent"),
              },
              { value: "unread", label: t("inbox.tab_unread") },
              { value: "all", label: t("inbox.tab_all") },
            ]}
          />
        </Tabs>
        ...
        <div className="flex items-center gap-2">
          ...
          <Button
            ...
            title={nestingEnabled ? t("inbox.nesting_disable") : t("inbox.nesting_enable")}
          >
          ...
          <Popover>
            ...
            <PopoverContent align="end" className="w-40 p-2">
              <div className="space-y-0.5">
                {([
                  ["none", t("inbox.group_none")],
                  ["type", t("inbox.group_type")],
                  ...(isolatedWorkspacesEnabled ? ([["workspace", t("inbox.group_workspace")]] as const) : []),
          ...
          {canMarkAllRead && (
            <>
              ...
              >
                {markAllReadMutation.isPending ? t("inbox.marking") : t("inbox.mark_all_read")}
              </Button>
              <Dialog open={showMarkAllReadConfirm} onOpenChange={setShowMarkAllReadConfirm}>
                <DialogContent className="sm:max-w-md">
                  <DialogHeader>
                    <DialogTitle>{t("inbox.mark_all_dialog_title")}</DialogTitle>
                    <DialogDescription>
                      {t("inbox.mark_all_dialog_body", { count: unreadIssueIds.length })}
                    </DialogDescription>
                  </DialogHeader>
                  <DialogFooter>
        ...
        <div className="flex flex-wrap items-center gap-2">
          ...
          >
            <SelectTrigger className="h-8 w-[170px] text-xs">
              <SelectValue placeholder={t("inbox.category_placeholder")} />
            </SelectTrigger>
            <SelectContent>
              <SelectItem value="everything">{t("inbox.category_everything")}</SelectItem>
              <SelectItem value="issues_i_touched">{t("inbox.category_my_recent")}</SelectItem>
              <SelectItem value="join_requests">{t("inbox.category_join_requests")}</SelectItem>
              <SelectItem value="approvals">{t("inbox.category_approvals")}</SelectItem>
              <SelectItem value="failed_runs">{t("inbox.category_failed_runs")}</SelectItem>
              <SelectItem value="alerts">{t("inbox.category_alerts")}</SelectItem>
            </SelectContent>
          </Select>
          ...
          {showApprovalsCategory && (
            ...
            >
              <SelectTrigger className="h-8 w-[170px] text-xs">
                <SelectValue placeholder={t("inbox.approval_status_placeholder")} />
              </SelectTrigger>
              <SelectContent>
                <SelectItem value="all">{t("inbox.approval_status_all")}</SelectItem>
                <SelectItem value="actionable">{t("inbox.approval_status_actionable")}</SelectItem>
                <SelectItem value="resolved">{t("inbox.approval_status_resolved")}</SelectItem>
              </SelectContent>
            </Select>
          )}
        </div>
      ...
      {allLoaded && visibleSections.length === 0 && (
        <EmptyState
          ...
          message={
            searchQuery.trim()
              ? t("inbox.empty_search")
              : tab === "mine"
              ? t("inbox.empty_mine")
              : tab === "unread"
              ? t("inbox.empty_unread")
              : tab === "recent"
                ? t("inbox.empty_recent")
                : t("inbox.empty_filtered")
          }
        />
      )}
                    ...
                    elements.push(
                      ...
                      >
                        ...
                        <span className="shrink-0 text-[11px] font-semibold uppercase tracking-wide text-muted-foreground">
                          {group.searchSection === "archived" ? t("inbox.search_section_archived") : t("inbox.search_section_other")}
                        </span>
                        <div className="h-px flex-1 bg-border/80" />
                      </div>,
                    );
              ...
              {showBudgetAlert && (
                <div className="group/alert relative flex items-center gap-3 px-4 py-3 transition-colors hover:bg-accent/50">
                  ...
                  >
                    ...
                    <span className="text-sm">
                      {t("inbox.budget_alert", { percent: dashboard!.costs.monthUtilizationPercent })}
                    </span>
                  </Link>
                  <button
                    type="button"
