...
export function NewAgent() {
  ...
  return (
    <div className="mx-auto max-w-2xl space-y-6">
      ...
      {/* MSPRO fork: subscription-only warning */}
      <div className="rounded-md border border-primary/30 bg-primary/5 px-4 py-3 text-xs">
        <div className="flex gap-2 items-start">
          <Shield className="h-4 w-4 mt-0.5 text-primary shrink-0" aria-hidden />
          <div>
            <div className="font-medium text-primary">MSPRO fork — subscription-only</div>
            <p className="text-muted-foreground mt-1">
              Этот билд работает исключительно через подписку Claude Max / Cursor Pro.
              API-ключи (ANTHROPIC_API_KEY, CURSOR_API_KEY, OPENAI_API_KEY) и AWS Bedrock
              заблокированы. Убедись что <code className="font-mono">claude</code> CLI залогинен
              на хосте через <code className="font-mono">claude login</code>.
            </p>
          </div>
        </div>
      </div>
