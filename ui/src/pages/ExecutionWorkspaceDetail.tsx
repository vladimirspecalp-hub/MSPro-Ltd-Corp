...
function parseWorkspaceRuntimeJson(value: string, errors: { jsonObject: string; invalidJson: string }) {
  const trimmed = value.trim();
...
function buildWorkspacePatch(initialState: WorkspaceFormState, nextState: WorkspaceFormState, errors: { jsonObject: string; invalidJson: string }) {
  ...
  if (initialState.inheritRuntime !== nextState.inheritRuntime || initialState.workspaceRuntime !== nextState.workspaceRuntime) {
    const parsed = parseWorkspaceRuntimeJson(nextState.workspaceRuntime, errors);
...
function validateForm(form: WorkspaceFormState, msgs: ValidationStrings) {
  ...
  if (!form.inheritRuntime) {
    const runtimeJson = parseWorkspaceRuntimeJson(form.workspaceRuntime, { jsonObject: msgs.jsonObject, invalidJson: msgs.invalidJson });
    if (!runtimeJson.ok) {
...
function MonoValue({ value, copy }: { value: string; copy?: boolean }) {
  return (
    <div className="inline-flex max-w-full items-start gap-2">
      ...
      {copy ? (
        <CopyText text={value} className="shrink-0 text-muted-foreground hover:text-foreground" copiedLabel="Copied" /* note: i18n inside CopyText handled separately */>
...
export function ExecutionWorkspaceDetail() {
  ...
  const validationMsgs: ValidationStrings = useMemo(() => ({
    repoUrl: t("execution_workspace.validation_repo_url"),
    jsonObject: t("execution_workspace.validation_json_object"),
    invalidJson: t("execution_workspace.validation_invalid_json"),
  }), [t]);
  ...
  useEffect(() => {
    if (!workspace) return;
    const crumbs = [
      { label: t("execution_workspace.breadcrumb_projects"), href: "/projects" },
      ...(project ? [{ label: project.name, href: `/projects/${projectRef}` }] : []),
      ...(project ? [{ label: t("execution_workspace.breadcrumb_workspaces"), href: `/projects/${projectRef}/workspaces` }] : []),
  ...
  const updateWorkspace = useMutation({
    ...
    onError: (error) => {
      setErrorMessage(error instanceof Error ? error.message : t("execution_workspace.save_failed_default"));
    },
  });
  ...
  const controlRuntimeServices = useMutation({
    ...
    onSuccess: (result, request) => {
      ...
      setRuntimeActionMessage(
        request.action === "run"
          ? t("execution_workspace.runtime_msg_run")
          : request.action === "stop"
            ? t("execution_workspace.runtime_msg_stop")
            : request.action === "restart"
              ? t("execution_workspace.runtime_msg_restart")
              : t("execution_workspace.runtime_msg_start"),
      );
    },
    onError: (error) => {
      setRuntimeActionMessage(null);
      setRuntimeActionErrorMessage(error instanceof Error ? error.message : t("execution_workspace.control_failed_default"));
    },
  });

  if (workspaceQuery.isLoading) return <p className="text-sm text-muted-foreground">{t("execution_workspace.loading_workspace")}</p>;
  if (workspaceQuery.error) {
    return (
      <p className="text-sm text-destructive">
        {workspaceQuery.error instanceof Error ? workspaceQuery.error.message : t("execution_workspace.load_failed")}
      </p>
    );
  }
  ...
  const saveChanges = () => {
    const validationError = validateForm(form, validationMsgs);
    if (validationError) {
    ...
    try {
      patch = buildWorkspacePatch(initialState, form, { jsonObject: validationMsgs.jsonObject, invalidJson: validationMsgs.invalidJson });
    } catch (error) {
      setErrorMessage(error instanceof Error ? error.message : t("execution_workspace.build_patch_failed_default"));
      return;
    }
      ...
      <div className="space-y-4 overflow-hidden sm:space-y-6">
        <div className="flex flex-wrap items-center gap-3">
          <Button variant="ghost" size="sm" asChild>
            <Link to={project ? `/projects/${projectRef}/workspaces` : "/projects"}>
              ...
              {t("execution_workspace.back_to_workspaces")}
            </Link>
          </Button>
        ...
        <div className="space-y-2">
          <div className="text-xs font-medium uppercase tracking-[0.16em] text-muted-foreground">
            {t("execution_workspace.kicker")}
          </div>
          <h1 className="truncate text-xl font-semibold sm:text-2xl">{workspace.name}</h1>
          <p className="max-w-2xl text-sm text-muted-foreground">
            {t("execution_workspace.intro")}
            <span className="hidden sm:inline">{t("execution_workspace.intro_extended")}</span>
          </p>
        </div>
        ...
        <div className="rounded-2xl border border-border bg-card p-4 sm:p-5">
          <div className="flex flex-col gap-4 sm:flex-row sm:items-start sm:justify-between">
            <div className="space-y-1">
              <div className="text-xs font-medium uppercase tracking-[0.16em] text-muted-foreground">{t("execution_workspace.commands_kicker")}</div>
              <h2 className="text-lg font-semibold">{t("execution_workspace.commands_title")}</h2>
              <p className="text-sm text-muted-foreground">
                {t("execution_workspace.source_label", { source: runtimeConfigSource === "execution_workspace"
                  ? t("execution_workspace.source_execution")
                  : runtimeConfigSource === "project_workspace"
                    ? t("execution_workspace.source_project")
                    : t("execution_workspace.source_none") })}
              </p>
            </div>
          </div>
          <WorkspaceRuntimeControls
            ...
            serviceEmptyMessage={
              effectiveRuntimeConfig
                ? t("execution_workspace.service_empty_with_runtime")
                : t("execution_workspace.service_empty_no_runtime")
            }
            jobEmptyMessage={t("execution_workspace.job_empty")}
            disabledHint={
              canStartRuntimeServices
                ? null
                : t("execution_workspace.disabled_hint")
            }
        ...
        <Tabs value={activeTab ?? "configuration"} onValueChange={(value) => handleTabChange(value as ExecutionWorkspaceTab)}>
          <PageTabBar
            items={[
              { value: "configuration", label: t("execution_workspace.tab_configuration") },
              { value: "runtime_logs", label: t("execution_workspace.tab_runtime_logs") },
              { value: "issues", label: t("execution_workspace.tab_issues") },
            ]}
            align="start"
        ...
        {activeTab === "configuration" ? (
          <div className="space-y-4 sm:space-y-6">
            <div className="rounded-2xl border border-border bg-card p-4 sm:p-5">
              <div className="flex flex-col gap-4 sm:flex-row sm:flex-wrap sm:items-start sm:justify-between">
                <div className="space-y-1">
                  <div className="text-xs font-medium uppercase tracking-[0.16em] text-muted-foreground">
                    {t("execution_workspace.configuration_kicker")}
                  </div>
                  <h2 className="text-lg font-semibold">{t("execution_workspace.configuration_title")}</h2>
                  <p className="text-sm text-muted-foreground">
                    {t("execution_workspace.configuration_intro")}
                  </p>
                </div>
                <Button
                  variant="outline"
                ...
                >
                  {workspace.status === "cleanup_failed" ? t("execution_workspace.retry_close") : t("execution_workspace.close_workspace")}
                </Button>
              </div>
              ...
              <div className="space-y-4">
                <Field label={t("execution_workspace.field_name")}>
                  <input
                    className="w-full rounded-lg border border-border bg-background px-3 py-2 text-sm outline-none"
                    value={form.name}
                    onChange={(event) => setForm((current) => current ? { ...current, name: event.target.value } : current)}
                    placeholder={t("execution_workspace.field_name_placeholder")}
                  />
                </Field>

                <Field label={t("execution_workspace.field_branch")} hint={t("execution_workspace.field_branch_hint")}>
                  <input
                    className="w-full rounded-lg border border-border bg-background px-3 py-2 font-mono text-sm outline-none"
                    value={form.branchName}
                    onChange={(event) => setForm((current) => current ? { ...current, branchName: event.target.value } : current)}
                    placeholder={t("execution_workspace.field_branch_placeholder")}
                  />
                </Field>

                <Field label={t("execution_workspace.field_cwd")}>
                  <input
                    className="w-full rounded-lg border border-border bg-background px-3 py-2 font-mono text-sm outline-none"
                    value={form.cwd}
                    onChange={(event) => setForm((current) => current ? { ...current, cwd: event.target.value } : current)}
                    placeholder={t("execution_workspace.field_cwd_placeholder")}
                  />
                </Field>

                <Field label={t("execution_workspace.field_provider_ref")}>
                  <input
                    className="w-full rounded-lg border border-border bg-background px-3 py-2 font-mono text-sm outline-none"
                    value={form.providerRef}
                    onChange={(event) => setForm((current) => current ? { ...current, providerRef: event.target.value } : current)}
                    placeholder={t("execution_workspace.field_provider_ref_placeholder")}
                  />
                </Field>

                <Field label={t("execution_workspace.field_repo_url")}>
                  <input
                    className="w-full rounded-lg border border-border bg-background px-3 py-2 text-sm outline-none"
                    value={form.repoUrl}
                    onChange={(event) => setForm((current) => current ? { ...current, repoUrl: event.target.value } : current)}
                    placeholder={t("execution_workspace.field_repo_url_placeholder")}
                  />
                </Field>

                <Field label={t("execution_workspace.field_base_ref")}>
                  <input
                    className="w-full rounded-lg border border-border bg-background px-3 py-2 font-mono text-sm outline-none"
                    value={form.baseRef}
                    onChange={(event) => setForm((current) => current ? { ...current, baseRef: event.target.value } : current)}
                    placeholder={t("execution_workspace.field_base_ref_placeholder")}
                  />
                </Field>

                <Field label={t("execution_workspace.field_provision")} hint={t("execution_workspace.field_provision_hint")}>
                  <textarea
                    ...
                    placeholder={t("execution_workspace.field_provision_placeholder")}
                  />
                </Field>

                <Field label={t("execution_workspace.field_teardown")} hint={t("execution_workspace.field_teardown_hint")}>
                  <textarea
                    className="min-h-20 w-full rounded-lg border border-border bg-background px-3 py-2 font-mono text-sm outline-none sm:min-h-28"
                    value={form.teardownCommand}
                    onChange={(event) => setForm((current) => current ? { ...current, teardownCommand: event.target.value } : current)}
                    placeholder={t("execution_workspace.field_teardown_placeholder")}
                  />
                </Field>

                <Field label={t("execution_workspace.field_cleanup")} hint={t("execution_workspace.field_cleanup_hint")}>
                  <textarea
                    ...
                    placeholder={t("execution_workspace.field_cleanup_placeholder")}
                  />
                </Field>
                ...
                <div className="rounded-xl border border-dashed border-border/70 bg-muted/20 px-3 py-3">
                  <div className="flex flex-col gap-3 sm:flex-row sm:flex-wrap sm:items-center sm:justify-between">
                    <div>
                      <div className="text-xs font-medium uppercase tracking-[0.16em] text-muted-foreground">
                        {t("execution_workspace.runtime_source_label")}
                      </div>
                      <p className="mt-1 text-sm text-muted-foreground">
                        {runtimeConfigSource === "execution_workspace"
                          ? t("execution_workspace.runtime_source_override")
                          : runtimeConfigSource === "project_workspace"
                            ? t("execution_workspace.runtime_source_inherit")
                            : t("execution_workspace.runtime_source_none")}
                      </p>
                    </div>
                    <Button
                      variant="outline"
                    ...
                    >
                      {t("execution_workspace.reset_to_inherit")}
                    </Button>
                  </div>
                </div>

                <details className="rounded-xl border border-dashed border-border/70 bg-muted/20 px-3 py-3">
                  <summary className="cursor-pointer text-sm font-medium">{t("execution_workspace.advanced_section")}</summary>
                  <p className="mt-2 text-sm text-muted-foreground">
                    {t("execution_workspace.advanced_intro")}
                  </p>
                  <div className="mt-3">
                    <Field label={t("execution_workspace.field_runtime_json")} hint={t("execution_workspace.field_runtime_json_hint")}>
                      <div className="mb-2 flex items-center gap-2 text-xs text-muted-foreground">
                        ...
                        />
                        <label htmlFor="inherit-runtime-config">{t("execution_workspace.inherit_runtime_label")}</label>
                      </div>
                      <textarea
              ...
              <div className="mt-5 flex flex-col items-stretch gap-3 sm:flex-row sm:flex-wrap sm:items-center">
                <Button className="w-full sm:w-auto" disabled={!isDirty || updateWorkspace.isPending} onClick={saveChanges}>
                  ...
                  {t("execution_workspace.save_changes")}
                </Button>
                <Button
                  variant="outline"
                  className="w-full sm:w-auto"
                ...
                >
                  {t("execution_workspace.reset")}
                </Button>
                {errorMessage ? <p className="text-sm text-destructive">{errorMessage}</p> : null}
                {!errorMessage && !isDirty ? <p className="text-sm text-muted-foreground">{t("execution_workspace.no_unsaved")}</p> : null}
              </div>
            </div>

            <div className="rounded-2xl border border-border bg-card p-4 sm:p-5">
              <div className="space-y-1">
                <div className="text-xs font-medium uppercase tracking-[0.16em] text-muted-foreground">{t("execution_workspace.linked_kicker")}</div>
                <h2 className="text-lg font-semibold">{t("execution_workspace.linked_title")}</h2>
              </div>
              <Separator className="my-4" />
              <DetailRow label={t("execution_workspace.row_project")}>
                {project ? <Link to={`/projects/${projectRef}`} className="hover:underline">{project.name}</Link> : <MonoValue value={workspace.projectId} />}
              </DetailRow>
              <DetailRow label={t("execution_workspace.row_project_workspace")}>
                {project && linkedProjectWorkspace ? (
                  ...
                  t("execution_workspace.row_value_none")
                )}
              </DetailRow>
              <DetailRow label={t("execution_workspace.row_source_issue")}>
                {sourceIssue ? (
              ...
              <DetailRow label={t("execution_workspace.row_derived_from")}>
                {derivedWorkspace ? (
              ...
              <DetailRow label={t("execution_workspace.row_workspace_id")}>
            ...
            <div className="rounded-2xl border border-border bg-card p-4 sm:p-5">
              <div className="space-y-1">
                <div className="text-xs font-medium uppercase tracking-[0.16em] text-muted-foreground">{t("execution_workspace.paths_kicker")}</div>
                <h2 className="text-lg font-semibold">{t("execution_workspace.paths_title")}</h2>
              </div>
              <Separator className="my-4" />
              <DetailRow label={t("execution_workspace.row_cwd")}>
                {workspace.cwd ? <MonoValue value={workspace.cwd} copy /> : t("execution_workspace.row_value_none")}
              </DetailRow>
              <DetailRow label={t("execution_workspace.row_provider_ref")}>
                {workspace.providerRef ? <MonoValue value={workspace.providerRef} copy /> : t("execution_workspace.row_value_none")}
              </DetailRow>
              <DetailRow label={t("execution_workspace.row_repo_url")}>
              ...
              <DetailRow label={t("execution_workspace.row_base_ref")}>
                {workspace.baseRef ? <MonoValue value={workspace.baseRef} copy /> : t("execution_workspace.row_value_none")}
              </DetailRow>
              <DetailRow label={t("execution_workspace.row_branch")}>
                {workspace.branchName ? <MonoValue value={workspace.branchName} copy /> : t("execution_workspace.row_value_none")}
              </DetailRow>
              <DetailRow label={t("execution_workspace.row_opened")}>{formatDateTime(workspace.openedAt)}</DetailRow>
              <DetailRow label={t("execution_workspace.row_last_used")}>{formatDateTime(workspace.lastUsedAt)}</DetailRow>
              <DetailRow label={t("execution_workspace.row_cleanup")}>
                {workspace.cleanupEligibleAt
                  ? `${formatDateTime(workspace.cleanupEligibleAt)}${workspace.cleanupReason ? ` · ${workspace.cleanupReason}` : ""}`
                  : t("execution_workspace.cleanup_not_scheduled")}
              </DetailRow>
            </div>
          </div>
          ...
          <div className="rounded-2xl border border-border bg-card p-4 sm:p-5">
            <div className="space-y-1">
              <div className="text-xs font-medium uppercase tracking-[0.16em] text-muted-foreground">{t("execution_workspace.logs_kicker")}</div>
              <h2 className="text-lg font-semibold">{t("execution_workspace.logs_title")}</h2>
            </div>
            <Separator className="my-4" />
            {workspaceOperationsQuery.isLoading ? (
              <p className="text-sm text-muted-foreground">{t("execution_workspace.loading_logs")}</p>
            ) : workspaceOperationsQuery.error ? (
              <p className="text-sm text-destructive">
                {workspaceOperationsQuery.error instanceof Error
                  ? workspaceOperationsQuery.error.message
                  : t("execution_workspace.logs_load_failed")}
              </p>
              ...
              <p className="text-sm text-muted-foreground">{t("execution_workspace.no_operations")}</p>
            )}
          </div>
        ) : (
