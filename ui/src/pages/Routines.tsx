...
export interface RoutineGroupLabels {
  noProject: string;
    ...
    >
      <div className="min-w-0 flex-1 space-y-1.5">
        <div className="flex flex-wrap items-center gap-2">
          ...
          {(isArchived || routine.status === "paused" || isDraft) ? (
            <span className="text-xs text-muted-foreground">
              {isArchived ? t("routines.state_archived_low") : isDraft ? t("routines.state_draft_low") : t("routines.state_paused_low")}
            </span>
          ) : null}
        </div>
        <div className="flex flex-wrap items-center gap-x-4 gap-y-1 text-xs text-muted-foreground">
          ...
          <span>
            {(() => {
              const time = formatLastRunTimestamp(routine.lastRun?.triggeredAt, t("routines.last_never"));
              const status = routine.lastRun ? formatRoutineRunStatus(routine.lastRun.status) : null;
              return status ? t("routines.last_with_status", { time, status }) : time;
            })()}
          </span>
        </div>
      </div>
      ...
      <div className="flex items-center gap-3" onClick={(event) => { event.preventDefault(); event.stopPropagation(); }}>
        <div className="flex items-center gap-3">
          <ToggleSwitch
            ...
            aria-label={enabled ? t("routines.aria_disable", { title: routine.title }) : t("routines.aria_enable", { title: routine.title })}
          />
          <span className="w-12 text-xs text-muted-foreground">
            {isArchived ? t("routines.state_archived") : isDraft ? t("routines.state_draft") : enabled ? t("routines.state_on") : t("routines.state_off")}
          </span>
        </div>

        <DropdownMenu>
          <DropdownMenuTrigger asChild>
            <Button variant="ghost" size="icon-sm" aria-label={t("routines.aria_more_actions", { title: routine.title })}>
              <MoreHorizontal className="h-4 w-4" />
            </Button>
          </DropdownMenuTrigger>
          <DropdownMenuContent align="end">
            <DropdownMenuItem asChild>
              <Link to={href}>{t("routines.menu_edit")}</Link>
            </DropdownMenuItem>
            <DropdownMenuItem
              disabled={runningRoutineId === routine.id || isArchived}
              onClick={() => onRunNow(routine)}
            >
              {runningRoutineId === routine.id ? t("routines.menu_running") : t("routines.menu_run_now")}
            </DropdownMenuItem>
            <DropdownMenuSeparator />
            <DropdownMenuItem
              ...
              disabled={isStatusPending || isArchived}
            >
              {enabled ? t("routines.menu_pause") : t("routines.menu_enable")}
            </DropdownMenuItem>
            <DropdownMenuItem
              onClick={() => onToggleArchived(routine)}
              disabled={isStatusPending}
            >
              {routine.status === "archived" ? t("routines.menu_restore") : t("routines.menu_archive")}
            </DropdownMenuItem>
          </DropdownMenuContent>
        </DropdownMenu>
      </div>
...
export function Routines() {
  ...
  const createRoutine = useMutation({
    ...
    onSuccess: async (routine) => {
      ...
      pushToast({
        title: t("routines.toast_created_title"),
        body: routine.assigneeAgentId
          ? t("routines.toast_created_body_with_agent")
          : t("routines.toast_created_body_draft"),
        tone: "success",
      });
  ...
  const updateRoutineStatus = useMutation({
    ...
    onError: (mutationError) => {
      pushToast({
        title: t("routines.toast_update_failed_title"),
        body: mutationError instanceof Error ? mutationError.message : t("routines.toast_update_failed_body"),
        tone: "error",
      });
    },
  });
  ...
  const runRoutine = useMutation({
    ...
    onError: (mutationError) => {
      pushToast({
        title: t("routines.toast_run_failed_title"),
        body: mutationError instanceof Error ? mutationError.message : t("routines.toast_run_failed_body"),
        tone: "error",
      });
    },
  });
  ...
  const groupLabels = useMemo<RoutineGroupLabels>(() => ({
    noProject: t("routines.no_project"),
    unknownProject: t("routines.unknown_project"),
    unassigned: t("routines.assignee_none"),
    unknownAgent: t("routines.unknown_agent"),
  }), [t]);
  const routineGroups = useMemo(
    () => buildRoutineGroups(routines ?? [], routineViewState.groupBy, projectById, agentById, groupLabels),
    [agentById, projectById, routineViewState.groupBy, routines, groupLabels],
  );
  const recentRunsIssueLinkState = useMemo(
    () =>
      createIssueDetailLocationState(
        t("routines.tab_recent_runs"),
  ...
  function handleToggleEnabled(routine: RoutineListItem, enabled: boolean) {
    if (!enabled && !routine.assigneeAgentId) {
      pushToast({
        title: t("routines.toast_default_agent_title"),
        body: t("routines.toast_default_agent_body"),
        tone: "warn",
      });
      return;
    }
    ...
    <div className="space-y-6">
      <div className="flex flex-col gap-3 sm:flex-row sm:items-end sm:justify-between">
        <div className="space-y-1">
          <h1 className="text-2xl font-semibold tracking-tight">
            {t("routines.page_title")}
          </h1>
          <p className="text-sm text-muted-foreground">
            {t("routines.page_subtitle")}
          </p>
        </div>
      ...
      <Tabs value={activeTab} onValueChange={handleTabChange}>
        <PageTabBar
          ...
          items={[
            { value: "routines", label: t("routines.tab_routines") },
            { value: "runs", label: t("routines.tab_recent_runs") },
          ]}
        />
            ...
            <Popover>
              <PopoverTrigger asChild>
                <Button variant="ghost" size="sm" className="text-xs">
                  ...
                  <span className="hidden sm:inline">{t("routines.group_label")}</span>
                </Button>
              </PopoverTrigger>
              <PopoverContent align="end" className="w-44 p-0">
                <div className="p-2 space-y-0.5">
                  {([
                    ["project", t("routines.group_project")],
                    ["assignee", t("routines.group_assignee")],
                    ["none", t("routines.group_none")],
      ...
      >
        ...
        >
          <div className="shrink-0 flex flex-wrap items-center justify-between gap-3 border-b border-border/60 px-5 py-3">
            <div>
              <p className="text-xs font-medium uppercase tracking-[0.2em] text-muted-foreground">{t("routines.composer_kicker")}</p>
              <p className="text-sm text-muted-foreground">
                {t("routines.composer_subtitle")}
              </p>
            </div>
            <Button
              variant="ghost"
              ...
              <div className="overflow-x-auto overscroll-x-contain">
                <div className="inline-flex min-w-full flex-wrap items-center gap-2 text-sm text-muted-foreground sm:min-w-max sm:flex-nowrap">
                  ...
                  <InlineEntitySelector
                    ...
                    placeholder={t("routines.assignee_placeholder")}
                    noneLabel={t("routines.assignee_none")}
                    searchPlaceholder={t("routines.assignee_search")}
                    emptyMessage={t("routines.assignee_empty")}
                    ...
                    renderTriggerValue={(option) =>
                      option ? (
                        ...
                        <span className="text-muted-foreground">{t("routines.assignee_placeholder")}</span>
                      )
                    }
                    renderOption={(option) => {
                  ...
                  <InlineEntitySelector
                    ...
                    placeholder={t("routines.project_placeholder")}
                    noneLabel={t("routines.project_none")}
                    searchPlaceholder={t("routines.project_search")}
                    emptyMessage={t("routines.project_empty")}
                    ...
                    renderTriggerValue={(option) =>
                      option && currentProject ? (
                        ...
                        <span className="text-muted-foreground">{t("routines.project_placeholder")}</span>
                      )
                    }
                    renderOption={(option) => {
                      if (!option.id) return <span className="truncate">{option.label}</span>;
            ...
            <div className="border-t border-border/60 px-5 py-3">
              <Collapsible open={advancedOpen} onOpenChange={setAdvancedOpen}>
                <CollapsibleTrigger className="flex w-full items-center justify-between text-left">
                  <div>
                    <p className="text-sm font-medium">{t("routines.advanced_title")}</p>
                    <p className="text-sm text-muted-foreground">{t("routines.advanced_subtitle")}</p>
                  </div>
                ...
                <CollapsibleContent className="pt-3">
                  <div className="grid gap-4 md:grid-cols-2">
                    <div className="space-y-2">
                      <p className="text-xs font-medium uppercase tracking-[0.18em] text-muted-foreground">{t("routines.field_concurrency")}</p>
                      <Select
                      ...
                      <p className="text-xs text-muted-foreground">{t(policyDescriptionKey[draft.concurrencyPolicy] ?? "")}</p>
                    </div>
                    <div className="space-y-2">
                      <p className="text-xs font-medium uppercase tracking-[0.18em] text-muted-foreground">{t("routines.field_catchup")}</p>
                      <Select
                      ...
                      <p className="text-xs text-muted-foreground">{t(policyDescriptionKey[draft.catchUpPolicy] ?? "")}</p>
                    </div>
                  </div>
                </CollapsibleContent>
              </Collapsible>
          ...
          <div className="shrink-0 flex flex-col gap-3 border-t border-border/60 px-5 py-4 sm:flex-row sm:items-center sm:justify-between">
            <div className="text-sm text-muted-foreground">
              {t("routines.composer_footer_note")}
            </div>
            <div className="flex flex-col gap-2 sm:items-end">
              ...
              >
                <Plus className="mr-2 h-4 w-4" />
                {createRoutine.isPending ? t("routines.submit_creating") : t("routines.submit_create")}
              </Button>
              {createRoutine.isError ? (
                <p className="text-sm text-destructive">
                  {createRoutine.error instanceof Error ? createRoutine.error.message : t("routines.create_failed_default")}
                </p>
              ) : null}
            </div>
          </div>
      ...
      {error ? (
        <Card>
          <CardContent className="pt-6 text-sm text-destructive">
            {error instanceof Error ? error.message : t("routines.failed_load")}
          </CardContent>
        </Card>
      ) : null}
