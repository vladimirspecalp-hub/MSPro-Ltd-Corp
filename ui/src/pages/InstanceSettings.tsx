...
import type { InstanceSchedulerHeartbeatAgent } from "@msproltd/shared";
...
export function InstanceSettings() {
  ...
  const toggleMutation = useMutation({
    ...
    onError: (error) => {
      setActionError(error instanceof Error ? error.message : t("instance.update_heartbeat_failed"));
    },
  });
  ...
  const disableAllMutation = useMutation({
    mutationFn: async (agentRows: InstanceSchedulerHeartbeatAgent[]) => {
      ...
      if (failures.length > 0) {
        ...
        const detail = firstError instanceof Error ? firstError.message : t("instance.unknown_error");
        throw new Error(
          failures.length === 1
            ? t("instance.disable_one_failed", { detail })
            : t("instance.disable_many_failed", { failed: failures.length, total: enabled.length, detail }),
        );
      }
      return enabled;
    },
    ...
    onError: (error) => {
      setActionError(error instanceof Error ? error.message : t("instance.disable_all_failed"));
    },
  });
  ...
  if (heartbeatsQuery.isLoading) {
    return <div className="text-sm text-muted-foreground">{t("instance.loading_heartbeats")}</div>;
  }
    ...
    return (
      <div className="text-sm text-destructive">
        {heartbeatsQuery.error instanceof Error
          ...
          : t("instance.load_heartbeats_failed")}
      </div>
    );
  }
    ...
    <div className="max-w-5xl space-y-6">
      <div className="space-y-2">
        <div className="flex items-center gap-2">
          ...
          <h1 className="text-lg font-semibold">{t("instance.heartbeats_title")}</h1>
        </div>
        <p className="text-sm text-muted-foreground">
          {t("instance.heartbeats_subtitle")}
        </p>
      </div>

      <div className="flex items-center gap-4 text-sm text-muted-foreground">
        <span><span className="font-semibold text-foreground">{activeCount}</span> {t("instance.stat_active")}</span>
        <span><span className="font-semibold text-foreground">{disabledCount}</span> {t("instance.stat_disabled")}</span>
        <span>{t("instance.stat_company", { count: grouped.length })}</span>
        {anyEnabled && (
          <Button
            variant="destructive"
            size="sm"
            ...
            onClick={() => {
              if (!window.confirm(t("instance.disable_all_confirm", { count: enabledCount }))) {
                return;
              }
              disableAllMutation.mutate(agents);
            }}
          >
            {disableAllMutation.isPending ? t("instance.disabling") : t("instance.disable_all")}
          </Button>
        )}
      </div>
      ...
      {agents.length === 0 ? (
        <EmptyState
          icon={Clock3}
          message={t("instance.no_heartbeats")}
        />
      ) : (
                  ...
                  {group.agents.map((agent) => {
                    ...
                    return (
                      ...
                      >
                        ...
                        >
                          {agent.schedulerActive ? t("instance.row_on") : t("instance.row_off")}
                        </Badge>
                        <Link
                        ...
                        >
                          {agent.lastHeartbeatAt
                            ...
                            : t("instance.row_never")}
                        </span>
                        <span className="ml-auto flex items-center gap-1.5 shrink-0">
                          <Link
                            to={buildAgentHref(agent)}
                            className="text-muted-foreground hover:text-foreground"
                            title={t("instance.agent_config_title")}
                          >
                          ...
                          >
                            {saving ? "…" : agent.heartbeatEnabled ? t("instance.row_disable_timer") : t("instance.row_enable_timer")}
                          </Button>
                        </span>
                      </div>
                    );
