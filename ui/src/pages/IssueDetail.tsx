...
function ActorIdentity({ evt, agentMap }: { evt: ActivityEvent; agentMap: Map<string, Agent> }) {
  ...
  if (evt.actorType === "system") return <Identity name={t("issue_detail.actor_system")} size="sm" />;
  if (evt.actorType === "user") return <Identity name={t("issue_detail.actor_board")} size="sm" />;
  return <Identity name={id || t("issue_detail.actor_unknown")} size="sm" />;
}
  ...
  return (
    <div className="flex items-center w-full">
      <Button
        ...
        aria-label={t("issue_detail.back_to_inbox")}
      >
      ...
      <div className="ml-auto flex items-center gap-0.5">
        {issueIdProp && !issueHidden && (
          <Button
            ...
            aria-label={t("issue_detail.archive_from_inbox")}
          >
        ...
        <Popover open={menuOpen} onOpenChange={setMenuOpen}>
          <PopoverTrigger asChild>
            <Button variant="ghost" size="icon-sm" aria-label={t("issue_detail.more_actions")}>
          ...
          <PopoverContent className="w-44 p-1" align="end">
            ...
            >
              ...
              {t("issue_detail.copy_as_markdown")}
            </button>
            <button
              className="flex items-center gap-2 w-full px-2 py-1.5 text-xs rounded hover:bg-accent/50"
            ...
            >
              ...
              {t("issue_detail.menu_properties")}
            </button>
            {issueIdProp && (
              <button
    ...
    <div className="space-y-3">
      {hasOlderComments ? (
        <div className="flex justify-center">
          ...
          >
            {commentsLoadingOlder ? t("issue_detail.earlier_comments_loading") : t("issue_detail.earlier_comments_load")}
          </Button>
        </div>
      ) : null}
      <IssueChatThread
    ...
    <>
      ...
      {linkedRuns && linkedRuns.length > 0 && (
        <div className="mb-3 px-3 py-2 rounded-lg border border-border">
          <div className="text-sm font-medium text-muted-foreground mb-1">{t("issue_detail.cost_summary")}</div>
          {!issueCostSummary.hasCost && !issueCostSummary.hasTokens ? (
            ...
            <div className="flex flex-wrap gap-3 text-xs text-muted-foreground tabular-nums">
              ...
              {issueCostSummary.hasTokens && (
                <span>
                  {t("issue_detail.tokens_label")} {formatTokens(issueCostSummary.totalTokens)}
                  {issueCostSummary.cached > 0
                    ? ` (${t("issue_detail.tokens_breakdown_cache", { input: formatTokens(issueCostSummary.input), output: formatTokens(issueCostSummary.output), cached: formatTokens(issueCostSummary.cached) })})`
                    : ` (${t("issue_detail.tokens_breakdown_no_cache", { input: formatTokens(issueCostSummary.input), output: formatTokens(issueCostSummary.output) })})`}
                </span>
              )}
            </div>
          )}
...
export function IssueDetail() {
  ...
  const breadcrumbTitle = issue?.title ?? issueId ?? t("issue_detail.breadcrumb_fallback");
  ...
  const updateIssue = useMutation({
    ...
    onError: (err, _variables, context) => {
      ...
      pushToast({
        title: t("issue_detail.toast_issue_update_failed_title"),
        body: err instanceof Error ? err.message : t("issue_detail.toast_issue_update_failed_body"),
        tone: "error",
      });
    },
    onSettled: () => {
  ...
  const updateChildIssue = useMutation({
    ...
    onError: (err) => {
      pushToast({
        title: t("issue_detail.toast_issue_update_failed_title"),
        body: err instanceof Error ? err.message : t("issue_detail.toast_sub_issue_update_failed_body"),
        tone: "error",
      });
    },
  });
  ...
  const approvalDecision = useMutation({
    ...
    onSuccess: (_approval, variables) => {
      ...
      pushToast({
        title: variables.action === "approve" ? t("issue_detail.toast_approval_approved") : t("issue_detail.toast_approval_rejected"),
        tone: "success",
      });
    },
    onError: (err, variables) => {
      pushToast({
        title: variables.action === "approve" ? t("issue_detail.toast_approval_failed") : t("issue_detail.toast_rejection_failed"),
        body: err instanceof Error ? err.message : t("issue_detail.toast_approval_update_body_default"),
        tone: "error",
      });
    },
    onSettled: () => {
  ...
  const addComment = useMutation({
    ...
    onSuccess: async (comment, _variables, context) => {
      ...
      if (context?.optimisticCommentId && cancelledQueuedOptimisticCommentIdsRef.current.has(context.optimisticCommentId)) {
        ...
        try {
          ...
          pushToast({
            title: t("issue_detail.toast_cancel_failed_title"),
            body: err instanceof Error ? err.message : t("issue_detail.toast_cancel_failed_body"),
            tone: "error",
          });
        }
      }
    ...
    onError: (err, _variables, context) => {
      ...
      pushToast({
        title: t("issue_detail.toast_comment_failed_title"),
        body: err instanceof Error ? err.message : t("issue_detail.toast_comment_failed_body_default"),
        tone: "error",
      });
    },
  ...
  const interruptQueuedComment = useMutation({
    ...
    onSuccess: () => {
      ...
      invalidateIssueRunState();
      pushToast({
        title: t("issue_detail.toast_interrupt_requested_title"),
        body: t("issue_detail.toast_interrupt_requested_body"),
        tone: "success",
      });
    },
    onError: (err, _runId, context) => {
      ...
      pushToast({
        title: t("issue_detail.toast_interrupt_failed_title"),
        body: err instanceof Error ? err.message : t("issue_detail.toast_interrupt_failed_body"),
        tone: "error",
      });
    },
  });
  ...
  const cancelQueuedComment = useMutation({
    ...
    onSuccess: (comment) => {
      ...
      invalidateIssueCollections();
      pushToast({
        title: t("issue_detail.toast_queued_canceled_title"),
        body: t("issue_detail.toast_queued_canceled_body"),
        tone: "success",
      });
    },
    onError: (err) => {
        ...
        title:
          variables.sharingPreferenceAtSubmit === "prompt"
            ? variables.allowSharing
              ? t("issue_detail.toast_feedback_saved_will_share")
              : t("issue_detail.toast_feedback_saved_will_local")
            : variables.allowSharing
              ? t("issue_detail.toast_feedback_saved_sharing_enabled")
              : t("issue_detail.toast_feedback_saved"),
        tone: "success",
      });
    },
    onError: (err, _variables, context) => {
      ...
      pushToast({
        title: t("issue_detail.toast_feedback_save_failed_title"),
        body: err instanceof Error ? err.message : t("issue_detail.toast_unknown_error"),
        tone: "error",
      });
    },
  });
  ...
  const uploadAttachment = useMutation({
    mutationFn: async (file: File) => {
      if (!selectedCompanyId) throw new Error(t("issue_detail.toast_no_company"));
    ...
    onError: (err) => {
      setAttachmentError(err instanceof Error ? err.message : t("issue_detail.toast_upload_failed"));
    },
  });
  ...
  const importMarkdownDocument = useMutation({
    ...
    onError: (err) => {
      setAttachmentError(err instanceof Error ? err.message : t("issue_detail.toast_document_import_failed"));
    },
  });
  ...
  const deleteAttachment = useMutation({
    ...
    onError: (err) => {
      setAttachmentError(err instanceof Error ? err.message : t("issue_detail.toast_delete_failed"));
    },
  });
  ...
  const archiveFromInbox = useMutation({
    ...
    onSuccess: () => {
      ...
      pushToast({ title: t("issue_detail.toast_archived_title"), tone: "success" });
    },
    onError: (err) => {
      pushToast({
        title: t("issue_detail.toast_archive_failed_title"),
        body: err instanceof Error ? err.message : t("issue_detail.toast_archive_failed_body"),
        tone: "error",
      });
    },
  });
  ...
  const copyIssueToClipboard = async () => {
    ...
    setCopied(true);
    pushToast({ title: t("issue_detail.copied_to_clipboard"), tone: "success" });
      ...
      >
        ...
        {uploadAttachment.isPending || importMarkdownDocument.isPending ? t("issue_detail.uploading") : (
          <>
            ...
            <span className="sm:hidden">{t("issue_detail.upload_short")}</span>
          </>
        )}
      </Button>
    </>
    ...
    <div className="max-w-2xl space-y-6">
      ...
      <div className="space-y-3">
        <div className="flex items-center gap-2 min-w-0 flex-wrap">
          ...
          {!(isMobile && isFromInbox) && (
            <div className="ml-auto flex items-center gap-0.5 md:hidden shrink-0">
              <Button
                ...
                title={t("issue_detail.copy_issue_title")}
              >
              ...
              <Button
                ...
                title={t("issue_detail.properties_title")}
              >
          ...
          <div className="hidden md:flex items-center md:ml-auto shrink-0">
            ...
            <Button
              ...
              title={t("issue_detail.show_properties_title")}
            >
              <SlidersHorizontal className="h-4 w-4" />
            </Button>
        ...
        <InlineEditor
          ...
          placeholder={t("issue_detail.description_placeholder")}
          multiline
      ...
      {showRichSubIssuesSection ? (
        <div className="space-y-3">
          <div className="flex items-center justify-between gap-2">
            <h3 className="text-sm font-medium text-muted-foreground">{t("issue_detail.sub_issues_title")}</h3>
          </div>
          <IssuesList
            issues={childIssues}
            ...
            createIssueLabel={t("issue_detail.sub_issue_label")}
      ...
      >
        <div className="flex items-center justify-between gap-2">
          <h3 className="text-sm font-medium text-muted-foreground">{t("issue_detail.attachments_title")}</h3>
            ...
            {imageAttachments.map((attachment) => (
              ...
              >
                ...
                {confirmDeleteId === attachment.id ? (
                  ...
                  >
                    <p className="text-xs text-white font-medium">{t("issue_detail.delete_question")}</p>
      ...
      <Tabs value={detailTab} onValueChange={setDetailTab} className="space-y-3">
        <TabsList variant="line" className="w-full justify-start gap-1">
          <TabsTrigger value="chat" className="gap-1.5">
            ...
            {t("issue_detail.tab_chat")}
          </TabsTrigger>
          <TabsTrigger value="activity" className="gap-1.5">
            <ActivityIcon className="h-3.5 w-3.5" />
            {t("issue_detail.tab_activity")}
          </TabsTrigger>
      ...
      <Sheet open={mobilePropsOpen} onOpenChange={setMobilePropsOpen}>
        <SheetContent side="bottom" className="max-h-[85dvh] pb-[env(safe-area-inset-bottom)]">
          <SheetHeader>
            <SheetTitle className="text-sm">{t("issue_detail.properties_title")}</SheetTitle>
          </SheetHeader>
