...
import type { RoutineTrigger, RoutineVariable } from "@msproltd/shared";
...
const policyDescriptionKey: Record<string, string> = {
  coalesce_if_active: "routines.policy_coalesce_if_active",
  always_enqueue: "routines.policy_always_enqueue",
  skip_if_active: "routines.policy_skip_if_active",
  skip_missed: "routines.policy_skip_missed",
  enqueue_missed_with_cap: "routines.policy_enqueue_missed_with_cap",
};
const signingModeKey: Record<string, string> = {
  bearer: "routine_detail.signing_bearer",
  hmac_sha256: "routine_detail.signing_hmac_sha256",
  github_hmac: "routine_detail.signing_github_hmac",
  none: "routine_detail.signing_none",
};
  ...
  return (
    <div className="rounded-lg border border-border p-4 space-y-4">
      <div className="flex items-center justify-between">
        ...
        <span className="text-xs text-muted-foreground">
          {trigger.kind === "schedule" && trigger.nextRunAt
            ? t("routine_detail.trigger_next_label", { time: new Date(trigger.nextRunAt).toLocaleString() })
            : trigger.kind === "webhook"
              ? t("routine_detail.trigger_kind_webhook")
              : t("routine_detail.trigger_kind_api")}
        </span>
      </div>

      <div className="grid gap-3 md:grid-cols-2">
        <div className="space-y-1.5">
          <Label className="text-xs">{t("routine_detail.trigger_field_label")}</Label>
          <Input
            value={draft.label}
        ...
        {trigger.kind === "schedule" && (
          <div className="md:col-span-2 space-y-1.5">
            <Label className="text-xs">{t("routine_detail.schedule_label")}</Label>
            <ScheduleEditor
        ...
        {trigger.kind === "webhook" && (
          <>
            <div className="space-y-1.5">
              <Label className="text-xs">{t("routine_detail.signing_mode_label")}</Label>
              <Select
            ...
            {!SIGNING_MODES_WITHOUT_REPLAY_WINDOW.has(draft.signingMode) && (
              <div className="space-y-1.5">
                <Label className="text-xs">{t("routine_detail.replay_window_label")}</Label>
                <Input
      ...
      <div className="flex flex-wrap items-center gap-2">
        {trigger.lastResult && <span className="text-xs text-muted-foreground">{t("routine_detail.trigger_last_label", { result: trigger.lastResult })}</span>}
        <div className="ml-auto flex items-center gap-2">
          {trigger.kind === "webhook" && (
            <Button variant="outline" size="sm" onClick={() => onRotate(trigger.id)}>
              <RefreshCw className="mr-1.5 h-3.5 w-3.5" />
              {t("routine_detail.rotate_secret")}
            </Button>
          )}
          <Button
            variant="outline"
...
export function RoutineDetail() {
  ...
  const copySecretValue = async (label: string, value: string) => {
    try {
      ...
      pushToast({
        ...
        body: error instanceof Error ? error.message : t("routine_detail.toast_clipboard_failed_body_default"),
        tone: "error",
      });
    }
  };
  ...
  const saveRoutine = useMutation({
    ...
    onError: (error) => {
      pushToast({
        title: t("routine_detail.toast_save_failed_title"),
        body: error instanceof Error ? error.message : t("routine_detail.toast_save_failed_body"),
        tone: "error",
      });
    },
  });
  ...
  const runRoutine = useMutation({
    ...
    onSuccess: async () => {
      pushToast({ title: t("routine_detail.toast_run_started"), tone: "success" });
    ...
    onError: (error) => {
      pushToast({
        title: t("routine_detail.toast_run_failed_title"),
        body: error instanceof Error ? error.message : t("routine_detail.toast_run_failed_body"),
        tone: "error",
      });
    },
  });
  ...
  const updateRoutineStatus = useMutation({
    ...
    onSuccess: async (_data, status) => {
      pushToast({
        title: t("routine_detail.toast_saved_title"),
        body: status === "paused" ? t("routine_detail.toast_saved_paused") : t("routine_detail.toast_saved_active"),
        tone: "success",
      });
      await Promise.all([
        queryClient.invalidateQueries({ queryKey: queryKeys.routines.detail(routineId!) }),
    ...
    onError: (error) => {
      pushToast({
        title: t("routine_detail.toast_status_failed_title"),
        body: error instanceof Error ? error.message : t("routine_detail.toast_status_failed_body"),
        tone: "error",
      });
    },
  });
  ...
  const createTrigger = useMutation({
    ...
    onSuccess: async (result) => {
      if (result.secretMaterial) {
        setSecretMessage({
          title: t("routine_detail.toast_webhook_created_title"),
          webhookUrl: result.secretMaterial.webhookUrl,
          webhookSecret: result.secretMaterial.webhookSecret,
        });
      } else {
        pushToast({
          title: t("routine_detail.toast_trigger_added_title"),
          body: t("routine_detail.toast_trigger_added_body"),
          tone: "success",
        });
      }
      await Promise.all([
    ...
    onError: (error) => {
      pushToast({
        title: t("routine_detail.toast_trigger_create_failed_title"),
        body: error instanceof Error ? error.message : t("routine_detail.toast_trigger_create_failed_body"),
        tone: "error",
      });
    },
  });
  ...
  const updateTrigger = useMutation({
    ...
    onSuccess: async () => {
      pushToast({
        title: t("routine_detail.toast_trigger_saved_title"),
        body: t("routine_detail.toast_trigger_saved_body"),
        tone: "success",
      });
      await Promise.all([
        queryClient.invalidateQueries({ queryKey: queryKeys.routines.detail(routineId!) }),
    ...
    onError: (error) => {
      pushToast({
        title: t("routine_detail.toast_trigger_update_failed_title"),
        body: error instanceof Error ? error.message : t("routine_detail.toast_trigger_update_failed_body"),
        tone: "error",
      });
    },
  });
  ...
  const deleteTrigger = useMutation({
    ...
    onSuccess: async () => {
      pushToast({
        title: t("routine_detail.toast_trigger_deleted_title"),
        tone: "success",
      });
      await Promise.all([
        queryClient.invalidateQueries({ queryKey: queryKeys.routines.detail(routineId!) }),
    ...
    onError: (error) => {
      pushToast({
        title: t("routine_detail.toast_trigger_delete_failed_title"),
        body: error instanceof Error ? error.message : t("routine_detail.toast_trigger_delete_failed_body"),
        tone: "error",
      });
    },
  });
  ...
  const rotateTrigger = useMutation({
    ...
    onSuccess: async (result) => {
      setSecretMessage({
        title: t("routine_detail.toast_secret_rotated"),
        webhookUrl: result.secretMaterial.webhookUrl,
        webhookSecret: result.secretMaterial.webhookSecret,
      });
      await Promise.all([
    ...
    onError: (error) => {
      pushToast({
        title: t("routine_detail.toast_secret_rotate_failed_title"),
        body: error instanceof Error ? error.message : t("routine_detail.toast_secret_rotate_failed_body"),
        tone: "error",
      });
    },
  });
  ...
  if (!selectedCompanyId) {
    return <EmptyState icon={Repeat} message={t("routine_detail.select_company")} />;
  }
  ...
  if (error || !routine) {
    return (
      <p className="pt-6 text-sm text-destructive">
        {error instanceof Error ? error.message : t("routine_detail.not_found")}
      </p>
    );
  }
  ...
  const automationLabel = routine.status === "archived"
    ? t("routine_detail.automation_archived")
    : !routine.assigneeAgentId
      ? t("routine_detail.automation_draft")
      : automationEnabled
        ? t("routine_detail.automation_active")
        : t("routine_detail.automation_paused");
    ...
    <div className="max-w-2xl space-y-6">
      ...
      <div className="flex items-start gap-4">
        <textarea
          ...
          placeholder={t("routine_detail.title_placeholder")}
          rows={1}
          ...
          <ToggleSwitch
            ...
            onCheckedChange={() => {
              if (!automationEnabled && !routine.assigneeAgentId) {
                pushToast({
                  title: t("routine_detail.toast_default_agent_title"),
                  body: t("routine_detail.toast_default_agent_body"),
                  tone: "warn",
                });
                return;
              }
            ...
            aria-label={automationEnabled ? t("routine_detail.aria_pause") : t("routine_detail.aria_enable")}
          />
      ...
      {secretMessage && (
        <div className="rounded-lg border border-blue-500/30 bg-blue-500/5 p-4 space-y-3 text-sm">
          <div>
            ...
            <p className="text-xs text-muted-foreground">{t("routine_detail.secret_save_warning")}</p>
          </div>
          <div className="space-y-2">
            <div className="flex items-center gap-2">
              <Input value={secretMessage.webhookUrl} readOnly className="flex-1" />
              <Button variant="outline" size="sm" onClick={() => copySecretValue(t("routine_detail.secret_url_copy_label"), secretMessage.webhookUrl)}>
                <Copy className="h-3.5 w-3.5 mr-1" />
                {t("routine_detail.secret_url_label")}
              </Button>
            </div>
            <div className="flex items-center gap-2">
              <Input value={secretMessage.webhookSecret} readOnly className="flex-1" />
              <Button variant="outline" size="sm" onClick={() => copySecretValue(t("routine_detail.secret_value_copy_label"), secretMessage.webhookSecret)}>
                <Copy className="h-3.5 w-3.5 mr-1" />
                {t("routine_detail.secret_value_label")}
              </Button>
            </div>
          </div>
        </div>
      ...
      {!routine.assigneeAgentId ? (
        <div className="rounded-lg border border-amber-500/30 bg-amber-500/5 p-4 text-sm text-amber-900 dark:text-amber-200">
          {t("routine_detail.default_agent_warning")}
        </div>
      ) : null}
      ...
      <div className="overflow-x-auto overscroll-x-contain">
        <div className="inline-flex min-w-full flex-wrap items-center gap-2 text-sm text-muted-foreground sm:min-w-max sm:flex-nowrap">
          <span>{t("routine_detail.for")}</span>
          <InlineEntitySelector
            ref={assigneeSelectorRef}
            value={editDraft.assigneeAgentId}
            options={assigneeOptions}
            placeholder={t("routine_detail.assignee_placeholder")}
            noneLabel={t("routine_detail.assignee_none")}
            searchPlaceholder={t("routine_detail.assignee_search")}
            emptyMessage={t("routine_detail.assignee_empty")}
            ...
            renderTriggerValue={(option) =>
              option ? (
                ...
                <span className="text-muted-foreground">{t("routine_detail.assignee_placeholder")}</span>
              )
            }
            renderOption={(option) => {
          ...
          />
          <span>{t("routine_detail.in")}</span>
          <InlineEntitySelector
            ref={projectSelectorRef}
            value={editDraft.projectId}
            options={projectOptions}
            placeholder={t("routine_detail.project_placeholder")}
            noneLabel={t("routine_detail.project_none")}
            searchPlaceholder={t("routine_detail.project_search")}
            emptyMessage={t("routine_detail.project_empty")}
            ...
            renderTriggerValue={(option) =>
              option && currentProject ? (
                ...
                <span className="text-muted-foreground">{t("routine_detail.project_placeholder")}</span>
              )
            }
            renderOption={(option) => {
              if (!option.id) return <span className="truncate">{option.label}</span>;
      ...
      <Collapsible open={advancedOpen} onOpenChange={setAdvancedOpen}>
        <CollapsibleTrigger className="flex w-full items-center justify-between text-left">
          <span className="text-sm font-medium">{t("routine_detail.advanced_title")}</span>
          {advancedOpen ? <ChevronDown className="h-4 w-4 text-muted-foreground" /> : <ChevronRight className="h-4 w-4 text-muted-foreground" />}
        </CollapsibleTrigger>
        <CollapsibleContent className="pt-3">
          <div className="grid gap-4 md:grid-cols-2">
            <div className="space-y-2">
              <p className="text-xs font-medium uppercase tracking-[0.18em] text-muted-foreground">{t("routine_detail.field_concurrency")}</p>
              <Select
              ...
              <p className="text-xs text-muted-foreground">{t(policyDescriptionKey[editDraft.concurrencyPolicy] ?? "")}</p>
            </div>
            <div className="space-y-2">
              <p className="text-xs font-medium uppercase tracking-[0.18em] text-muted-foreground">{t("routine_detail.field_catchup")}</p>
              <Select
              ...
              <p className="text-xs text-muted-foreground">{t(policyDescriptionKey[editDraft.catchUpPolicy] ?? "")}</p>
            </div>
          </div>
        </CollapsibleContent>
      </Collapsible>
      ...
      <div className="flex items-center justify-between">
        {isEditDirty ? (
          <span className="text-xs text-amber-600">{t("routine_detail.unsaved_changes")}</span>
        ) : (
          <span />
        )}
        <Button
...
<TabsTrigger value="activity" className="gap-1.5">
        ...
        <TabsContent value="triggers" className="space-y-4">
          ...
          <div className="rounded-lg border border-border p-4 space-y-3">
            <p className="text-sm font-medium">{t("routine_detail.add_trigger")}</p>
            <div className="grid gap-3 md:grid-cols-2">
              <div className="space-y-1.5">
                <Label className="text-xs">{t("routine_detail.add_trigger_label")}</Label>
                  ...
                  <SelectContent>
                    {triggerKinds.map((kind) => (
                      <SelectItem key={kind} value={kind} disabled={kind === "webhook"}>
                        {kind === "webhook" ? t("routine_detail.trigger_kind_coming_soon", { kind }) : kind}
                      </SelectItem>
                    ))}
                  </SelectContent>
                </Select>
              ...
              {newTrigger.kind === "webhook" && (
                <>
                  <div className="space-y-1.5">
                    ...
                    <p className="text-xs text-muted-foreground">{t(signingModeKey[newTrigger.signingMode] ?? "")}</p>
                  </div>
          ...
          {routine.triggers.length === 0 ? (
            <p className="text-xs text-muted-foreground">{t("routine_detail.no_triggers")}</p>
          ) : (
        ...
        <TabsContent value="runs" className="space-y-4">
          ...
          {(routineRuns ?? []).length === 0 ? (
            <p className="text-xs text-muted-foreground">{t("routine_detail.no_runs")}</p>
          ) : (
        ...
        <TabsContent value="activity">
          {(activity ?? []).length === 0 ? (
            <p className="text-xs text-muted-foreground">{t("routine_detail.no_activity")}</p>
          ) : (
            <div className="border border-border rounded-lg divide-y divide-border">
              {(activity ?? []).map((event) => (
