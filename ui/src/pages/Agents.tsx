...
export function Agents() {
  ...
  if (!selectedCompanyId) {
    return <EmptyState icon={Bot} message={t("agents_page.select_company")} />;
  }
    ...
    <div className="space-y-4">
      <div className="flex flex-col gap-3 sm:flex-row sm:items-center sm:justify-between">
        <Tabs value={tab} onValueChange={(v) => navigate(`/agents/${v}`)}>
          <PageTabBar
            items={[
              { value: "all", label: t("agents_page.all") },
              { value: "active", label: t("agents_page.active") },
              { value: "paused", label: t("agents_page.paused") },
              { value: "error", label: t("agents_page.error") },
            ]}
            value={tab}
      ...
      {filtered.length > 0 && (
        <p className="text-xs text-muted-foreground">{t("agents_page.agents_count", { count: filtered.length })}</p>
      )}
