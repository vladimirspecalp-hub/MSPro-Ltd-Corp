...
describe("worktree config repair", () => {
  it("repairs legacy repo-local worktree config and env files into an isolated instance", async () => {
    const tempRoot = await fs.mkdtemp(path.join(os.tmpdir(), "mspro-ltd-worktree-repair-"));
    const worktreeRoot = path.join(tempRoot, "PAP-884-ai-commits-component");
    const paperclipDir = path.join(worktreeRoot, ".mspro-ltd");
    const configPath = path.join(paperclipDir, "config.json");
    const envPath = path.join(paperclipDir, ".env");
    const sharedRoot = path.join(tempRoot, ".mspro-ltd", "instances", "default");
    const isolatedHome = path.join(tempRoot, ".mspro-ltd-worktrees");
    ...
    await fs.writeFile(
      envPath,
      [
        "# MSProLtd environment variables",
        "MSPROLTD_IN_WORKTREE=true",
        "MSPROLTD_WORKTREE_NAME=PAP-884-ai-commits-component",
        "MSPROLTD_AGENT_JWT_SECRET=shared-secret",
        "",
      ].join("\n"),
      "utf8",
    );
    ...
    process.env.MSPROLTD_IN_WORKTREE = "true";
    process.env.MSPROLTD_WORKTREE_NAME = "PAP-884-ai-commits-component";
    process.env.MSPROLTD_WORKTREES_DIR = isolatedHome;
    ...
    expect(repairedEnv).toContain(`MSPROLTD_HOME=${JSON.stringify(isolatedHome)}`);
    expect(repairedEnv).toContain('MSPROLTD_INSTANCE_ID="pap-884-ai-commits-component"');
    expect(repairedEnv).toContain(`MSPROLTD_CONFIG=${JSON.stringify(await fs.realpath(configPath))}`);
    expect(repairedEnv).toContain(`MSPROLTD_CONTEXT=${JSON.stringify(path.join(isolatedHome, "context.json"))}`);
    expect(repairedEnv).toContain('MSPROLTD_AGENT_JWT_SECRET="shared-secret"');
    expect(process.env.MSPROLTD_HOME).toBe(isolatedHome);
    expect(process.env.MSPROLTD_INSTANCE_ID).toBe("pap-884-ai-commits-component");
  });

  it("avoids sibling worktree ports when repairing legacy configs", async () => {
    const tempRoot = await fs.mkdtemp(path.join(os.tmpdir(), "mspro-ltd-worktree-repair-ports-"));
    ...
    await fs.writeFile(
      ...
      [
        "# MSProLtd environment variables",
        "MSPROLTD_IN_WORKTREE=true",
        "MSPROLTD_WORKTREE_NAME=PAP-880-thumbs-capture-for-evals-feature",
        "",
      ].join("\n"),
      "utf8",
    );
    ...
    process.chdir(worktreeRoot);
    process.env.MSPROLTD_IN_WORKTREE = "true";
    process.env.MSPROLTD_WORKTREE_NAME = "PAP-880-thumbs-capture-for-evals-feature";
    process.env.MSPROLTD_WORKTREES_DIR = isolatedHome;
  ...
  it("rebalances duplicate ports for already isolated worktree configs", async () => {
    const tempRoot = await fs.mkdtemp(path.join(os.tmpdir(), "mspro-ltd-worktree-rebalance-"));
    const isolatedHome = path.join(tempRoot, ".mspro-ltd-worktrees");
    const repoWorktreesRoot = path.join(tempRoot, "repo", ".mspro-ltd", "worktrees");
    ...
    const paperclipDir = path.join(currentWorktreeRoot, ".mspro-ltd");
    const configPath = path.join(paperclipDir, "config.json");
    const envPath = path.join(paperclipDir, ".env");
    const currentInstanceRoot = path.join(isolatedHome, "instances", "pap-884-ai-commits-component");
    const siblingConfigPath = path.join(siblingWorktreeRoot, ".mspro-ltd", "config.json");
  ...
  it("persists runtime-selected worktree ports back into config", async () => {
    const tempRoot = await fs.mkdtemp(path.join(os.tmpdir(), "mspro-ltd-worktree-ports-"));
    ...
    process.chdir(worktreeRoot);
    process.env.MSPROLTD_IN_WORKTREE = "true";
    process.env.MSPROLTD_WORKTREE_NAME = "PAP-878-create-a-mine-tab-in-inbox";
    process.env.MSPROLTD_HOME = isolatedHome;
    process.env.MSPROLTD_INSTANCE_ID = "pap-878-create-a-mine-tab-in-inbox";
