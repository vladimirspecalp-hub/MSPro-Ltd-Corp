...
describe("agent local JWT", () => {
  const secretEnv = "MSPROLTD_AGENT_JWT_SECRET";
  const betterAuthSecretEnv = "BETTER_AUTH_SECRET";
  const ttlEnv = "MSPROLTD_AGENT_JWT_TTL_SECONDS";
  const issuerEnv = "MSPROLTD_AGENT_JWT_ISSUER";
  const audienceEnv = "MSPROLTD_AGENT_JWT_AUDIENCE";
  ...
  it("falls back to BETTER_AUTH_SECRET when MSPROLTD_AGENT_JWT_SECRET is absent", () => {
  ...
  it("rejects issuer/audience mismatch", () => {
    ...
    process.env[issuerEnv] = "mspro-ltd";
    process.env[audienceEnv] = "mspro-ltd-api";
    expect(verifyLocalAgentJwt(token!)).toBeNull();
  });
});
