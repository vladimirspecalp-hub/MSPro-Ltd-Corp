...
describe("forbidden token check", () => {
  it("derives username tokens without relying on whoami", () => {
    const tokens = resolveDynamicForbiddenTokens(
      { USER: "mspro-ltd", LOGNAME: "mspro-ltd", USERNAME: "pc" },
      {
        userInfo: () => ({ username: "mspro-ltd" }),
      },
    );

    expect(tokens).toEqual(["mspro-ltd", "pc"]);
  });
  ...
  it("merges dynamic and file-based forbidden tokens", async () => {
    ...
    try {
      const tokens = resolveForbiddenTokens(tokensFile, { USER: "mspro-ltd" }, {
        userInfo: () => ({ username: "mspro-ltd" }),
      });

      expect(tokens).toEqual(["mspro-ltd", "custom-token"]);
    } finally {
