...
describe("buildInviteOnboardingTextDocument", () => {
  it("renders a plain-text onboarding doc with expected endpoint references", () => {
    ...
    expect(text).toContain("MSProLtd OpenClaw Gateway Onboarding");
    ...
    expect(text).toContain("/api/invites/token-123/skills/mspro-ltd");
    expect(text).toContain("Suggested MSProLtd base URLs to try");
    ...
    expect(text).toContain("~/.openclaw/workspace/mspro-ltd-claimed-api-key.json");
    expect(text).toContain("MSPROLTD_API_KEY");
