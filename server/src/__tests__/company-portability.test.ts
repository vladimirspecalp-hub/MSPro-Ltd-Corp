...
import type { CompanyPortabilityFileEntry } from "@msproltd/shared";
...
describe("company portability", () => {
    ...
    const companySkills = [
      {
        ...
        slug: "mspro-ltd",
        name: "mspro-ltd",
        description: "MSProLtd coordination skill",
        markdown: "---\nname: mspro-ltd\ndescription: MSProLtd coordination skill\n---\n\n# MSProLtd\n",
        sourceType: "github",
        sourceLocator: "https://github.com/vladimirspecalp-hub/mspro-ltd/tree/master/skills/mspro-ltd",
  ...
  it("parses canonical GitHub import URLs with explicit ref and package path", () => {
    expect(
      parseGitHubSourceUrl("https://github.com/vladimirspecalp-hub/companies?ref=feature%2Fdemo&path=gstack"),
    ).toEqual({
      hostname: "github.com",
      owner: "msproltdai",
      repo: "companies",
  ...
  it("parses canonical GitHub import URLs with explicit companyPath", () => {
    expect(
      parseGitHubSourceUrl(
        "https://github.com/vladimirspecalp-hub/companies?ref=abc123&companyPath=gstack%2FCOMPANY.md",
      ),
    ).toEqual({
      hostname: "github.com",
      owner: "msproltdai",
  ...
  it("exports referenced skills as stubs by default with sanitized MSProLtd extension data", async () => {
    ...
    expect(asTextFile(exported.files["COMPANY.md"])).toContain('name: "MSProLtd"');
    ...
    expect(asTextFile(exported.files["skills/msproltdai/mspro-ltd/mspro-ltd/SKILL.md"])).toContain("metadata:");
    expect(asTextFile(exported.files["skills/msproltdai/mspro-ltd/mspro-ltd/SKILL.md"])).toContain('kind: "github-dir"');
    expect(exported.files["skills/msproltdai/mspro-ltd/mspro-ltd/references/api.md"]).toBeUndefined();
    ...
    const extension = asTextFile(exported.files[".mspro-ltd.yaml"]);
    expect(extension).toContain('schema: "mspro-ltd/v1"');
  ...
  it("exports default sidebar order into the MSProLtd extension and manifest", async () => {
    const portability = companyPortabilityService({} as any);
    ...
    expect(asTextFile(exported.files[".mspro-ltd.yaml"])).toContain([
      "sidebar:",
      "  agents:",
      '    - "claudecoder"',
      '    - "cmo"',
  ...
  it("expands referenced skills when requested", async () => {
    ...
    expect(asTextFile(exported.files["skills/msproltdai/mspro-ltd/mspro-ltd/SKILL.md"])).toContain("# MSProLtd");
    expect(asTextFile(exported.files["skills/msproltdai/mspro-ltd/mspro-ltd/SKILL.md"])).toContain("metadata:");
    expect(asTextFile(exported.files["skills/msproltdai/mspro-ltd/mspro-ltd/references/api.md"])).toContain("# API");
  });
  ...
  it("exports only selected skills when skills filter is provided", async () => {
    ...
    expect(asTextFile(exported.files["skills/company/PAP/company-playbook/SKILL.md"])).toContain("# Company Playbook");
    expect(exported.files["skills/msproltdai/mspro-ltd/mspro-ltd/SKILL.md"]).toBeUndefined();
  });
  ...
  it("warns and exports all skills when skills filter matches nothing", async () => {
    ...
    expect(exported.files["skills/company/PAP/company-playbook/SKILL.md"]).toBeDefined();
    expect(exported.files["skills/msproltdai/mspro-ltd/mspro-ltd/SKILL.md"]).toBeDefined();
  });

  it("exports the company logo into images/ and references it from .mspro-ltd.yaml", async () => {
    const storage = {
    ...
    expect(exported.files[".mspro-ltd.yaml"]).toContain('logoPath: "images/company-logo.png"');
  });
  ...
  it("exports duplicate skill slugs into readable namespaced paths", async () => {
    ...
    companySkillSvc.listFull.mockResolvedValue([
      ...
      },
      {
        id: "skill-mspro-ltd",
        companyId: "company-1",
        key: "msproltdai/mspro-ltd/release-changelog",
        slug: "release-changelog",
        name: "release-changelog",
        description: "Bundled release changelog skill",
        markdown: "---\nname: release-changelog\n---\n\n# Bundled Release Changelog\n",
        sourceType: "github",
        sourceLocator: "https://github.com/vladimirspecalp-hub/mspro-ltd/tree/master/skills/release-changelog",
        sourceRef: "0123456789abcdef0123456789abcdef01234567",
        trustLevel: "markdown_only",
        compatibility: "compatible",
        fileInventory: [{ path: "SKILL.md", kind: "skill" }],
    ...
    expect(asTextFile(exported.files["skills/msproltdai/mspro-ltd/release-changelog/SKILL.md"])).toContain("metadata:");
    expect(asTextFile(exported.files["skills/msproltdai/mspro-ltd/release-changelog/SKILL.md"])).toContain("msproltdai/mspro-ltd/release-changelog");
  });
  ...
  it("exports portable project workspace metadata and remaps it on import", async () => {
        ...
        workspaces: [
          {
            ...
            name: "Main Repo",
            sourceType: "git_repo",
            cwd: "/Users/dotta/mspro-ltd",
            repoUrl: "https://github.com/vladimirspecalp-hub/mspro-ltd.git",
            repoRef: "main",
            defaultRef: "main",
            visibility: "default",
            setupCommand: "pnpm install",
            cleanupCommand: "rm -rf .mspro-ltd-tmp",
            remoteProvider: null,
    ...
    expect(extension).toContain('repoUrl: "https://github.com/vladimirspecalp-hub/mspro-ltd.git"');
    expect(extension).toContain('defaultProjectWorkspaceKey: "main-repo"');
    expect(extension).toContain('projectWorkspaceKey: "main-repo"');
    expect(extension).not.toContain("/Users/dotta/mspro-ltd");
    ...
    await portability.importBundle({
      ...
      target: {
        mode: "new_company",
        newCompanyName: "Imported MSProLtd",
      },
  ...
  it("infers portable git metadata from a local checkout without task warning fan-out", async () => {
    const portability = companyPortabilityService({} as any);
    const repoDir = await fs.mkdtemp(path.join(os.tmpdir(), "mspro-ltd-portability-git-"));
    execFileSync("git", ["init"], { cwd: repoDir, stdio: "ignore" });
    execFileSync("git", ["checkout", "-b", "main"], { cwd: repoDir, stdio: "ignore" });
    execFileSync("git", ["remote", "add", "origin", "https://github.com/vladimirspecalp-hub/mspro-ltd.git"], {
      cwd: repoDir,
      stdio: "ignore",
    });
    ...
    const extension = asTextFile(exported.files[".mspro-ltd.yaml"]);
    expect(extension).toContain('repoUrl: "https://github.com/vladimirspecalp-hub/mspro-ltd.git"');
    expect(extension).toContain('projectWorkspaceKey: "mspro-ltd"');
  ...
  it("reads env inputs back from .mspro-ltd.yaml during preview import", async () => {
    const portability = companyPortabilityService({} as any);
  ...
  it("reads project env inputs back from .mspro-ltd.yaml during preview import", async () => {
    const portability = companyPortabilityService({} as any);
  ...
  it("exports routines as recurring task packages with MSProLtd routine extensions", async () => {
    const portability = companyPortabilityService({} as any);
  ...
  it("imports recurring task packages as routines instead of one-time issues", async () => {
    ...
    const preview = await portability.previewImport({
      source: { type: "inline", rootPath: "mspro-ltd-demo", files },
      include: { company: true, agents: true, projects: true, issues: true, skills: false },
      target: { mode: "new_company", newCompanyName: "Imported MSProLtd" },
      agents: "all",
      collisionStrategy: "rename",
    });
  ...
  it("migrates legacy schedule.recurrence imports into routine triggers", async () => {
    ...
    const files = {
      "COMPANY.md": ['---', 'schema: "agentcompanies/v1"', 'name: "Imported MSProLtd"', "---", ""].join("\n"),
  ...
  it("imports a vendor-neutral package without .mspro-ltd.yaml", async () => {
    const portability = companyPortabilityService({} as any);
    ...
    expect(preview.errors).toEqual([]);
    expect(preview.manifest.company?.name).toBe("Imported MSProLtd");
  ...
  it("imports a packaged company logo and attaches it to the target company", async () => {
    ...
    exported.files[".mspro-ltd.yaml"] = `${exported.files[".mspro-ltd.yaml"]}`.replace(
      'brandColor: "#5c5fff"\n',
