...
import type { BindMode, DeploymentExposure, DeploymentMode } from "@msproltd/shared";
...
function resolveAgentJwtSecretStatus(
  ...
  const envValue = process.env.MSPROLTD_AGENT_JWT_SECRET?.trim();
  if (envValue) {
    return {
      status: "pass",
      message: "set",
  ...
  if (existsSync(envFilePath)) {
    ...
    const fileValue = typeof parsed.MSPROLTD_AGENT_JWT_SECRET === "string" ? parsed.MSPROLTD_AGENT_JWT_SECRET.trim() : "";
    if (fileValue) {
      return {
        status: "warn",
  ...
  return {
    status: "warn",
    message: "missing (run `pnpm msproltdai onboard`)",
  };
}
