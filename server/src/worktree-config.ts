...
import type { PaperclipConfig } from "@msproltd/shared";
...
function formatEnvEntries(entries: Record<string, string>): string {
  return [
    ...
    "# Generated by MSProLtd worktree repair",
...
function resolveWorktreeRuntimeContext(
  ...
  if (env.MSPROLTD_IN_WORKTREE !== "true") return null;
  ...
  const worktreeName = nonEmpty(env.MSPROLTD_WORKTREE_NAME) ?? path.basename(worktreeRoot);
  const instanceId = nonEmpty(env.MSPROLTD_INSTANCE_ID) ?? sanitizeWorktreeInstanceId(worktreeName);
  const homeDir = resolveHomeAwarePath(
    nonEmpty(env.MSPROLTD_HOME) ??
      nonEmpty(env.MSPROLTD_WORKTREES_DIR) ??
      "~/.mspro-ltd-worktrees",
  );
...
function collectSiblingWorktreePorts(context: WorktreeRuntimeContext): {
  ...
  if (repoManagedWorktreesRoot && fs.existsSync(repoManagedWorktreesRoot)) {
    for (const entry of fs.readdirSync(repoManagedWorktreesRoot, { withFileTypes: true })) {
      ...
      const siblingConfigPath = path.resolve(repoManagedWorktreesRoot, entry.name, ".mspro-ltd", "config.json");
...
export function maybeRepairLegacyWorktreeConfigAndEnvFiles(): {
  ...
  process.env.MSPROLTD_HOME = context.homeDir;
  process.env.MSPROLTD_INSTANCE_ID = context.instanceId;
  process.env.MSPROLTD_CONFIG = context.configPath;
  process.env.MSPROLTD_CONTEXT = context.contextPath;
  process.env.MSPROLTD_WORKTREE_NAME = context.worktreeName;
  ...
  const desiredEnvEntries: Record<string, string> = {
    ...existingEnvEntries,
    MSPROLTD_HOME: context.homeDir,
    MSPROLTD_INSTANCE_ID: context.instanceId,
    MSPROLTD_CONFIG: context.configPath,
    MSPROLTD_CONTEXT: context.contextPath,
    MSPROLTD_IN_WORKTREE: "true",
    MSPROLTD_WORKTREE_NAME: context.worktreeName,
  };
