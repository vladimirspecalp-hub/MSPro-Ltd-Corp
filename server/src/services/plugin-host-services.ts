...
import { pluginLogs, agentTaskSessions as agentTaskSessionsTable } from "@msproltd/db";
 ...
 *
 * This implementation delegates to the core MSProLtd domain services,
  ...
  return {
    ...
    events: {
      ...
      async subscribe(params: { eventPattern: string; filter?: Record<string, unknown> | null }) {
        const handler = async (event: import("@msproltd/plugin-sdk").PluginEvent) => {
          if (notifyWorker) {
