...
import type { AdapterRuntimeServiceReport } from "@msproltd/adapter-utils";
import type { Db } from "@msproltd/db";
import { executionWorkspaces, projectWorkspaces, workspaceRuntimeServices } from "@msproltd/db";
import {
...
function buildWorkspaceCommandEnv(input: {
  ...
  env.MSPROLTD_WORKSPACE_CWD = input.worktreePath;
  env.MSPROLTD_WORKSPACE_PATH = input.worktreePath;
  env.MSPROLTD_WORKSPACE_WORKTREE_PATH = input.worktreePath;
  env.MSPROLTD_WORKSPACE_BRANCH = input.branchName;
  env.MSPROLTD_WORKSPACE_BASE_CWD = input.base.baseCwd;
  env.MSPROLTD_WORKSPACE_REPO_ROOT = input.repoRoot;
  env.MSPROLTD_WORKSPACE_SOURCE = input.base.source;
  env.MSPROLTD_WORKSPACE_REPO_REF = input.base.repoRef ?? "";
  env.MSPROLTD_WORKSPACE_REPO_URL = input.base.repoUrl ?? "";
  env.MSPROLTD_WORKSPACE_CREATED = input.created ? "true" : "false";
  env.MSPROLTD_PROJECT_ID = input.base.projectId ?? "";
  env.MSPROLTD_PROJECT_WORKSPACE_ID = input.base.workspaceId ?? "";
  env.MSPROLTD_AGENT_ID = input.agent.id ?? "";
  env.MSPROLTD_AGENT_NAME = input.agent.name;
  env.MSPROLTD_COMPANY_ID = input.agent.companyId;
  env.MSPROLTD_ISSUE_ID = input.issue?.id ?? "";
  env.MSPROLTD_ISSUE_IDENTIFIER = input.issue?.identifier ?? "";
  env.MSPROLTD_ISSUE_TITLE = input.issue?.title ?? "";
  return env;
}
...
function buildExecutionWorkspaceCleanupEnv(input: {
  ...
  env.MSPROLTD_WORKSPACE_CWD = input.workspace.cwd ?? "";
  env.MSPROLTD_WORKSPACE_PATH = input.workspace.cwd ?? "";
  env.MSPROLTD_WORKSPACE_WORKTREE_PATH =
    input.workspace.providerRef ?? input.workspace.cwd ?? "";
  env.MSPROLTD_WORKSPACE_BRANCH = input.workspace.branchName ?? "";
  env.MSPROLTD_WORKSPACE_BASE_CWD = input.projectWorkspaceCwd ?? "";
  env.MSPROLTD_WORKSPACE_REPO_ROOT = input.projectWorkspaceCwd ?? "";
  env.MSPROLTD_WORKSPACE_REPO_URL = input.workspace.repoUrl ?? "";
  env.MSPROLTD_WORKSPACE_REPO_REF = input.workspace.baseRef ?? "";
  env.MSPROLTD_PROJECT_ID = input.workspace.projectId ?? "";
  env.MSPROLTD_PROJECT_WORKSPACE_ID = input.workspace.projectWorkspaceId ?? "";
  env.MSPROLTD_ISSUE_ID = input.workspace.sourceIssueId ?? "";
  return env;
}
...
export async function realizeExecutionWorkspace(input: {
  ...
  const worktreeParentDir = configuredParentDir
    ...
    : path.join(repoRoot, ".mspro-ltd", "worktrees");
  ...
  for (const row of projectWorkspaceRows) {
    ...
    try {
      const refs = await startRuntimeServicesForWorkspaceControl({
        db,
        actor: { id: null, name: "MSProLtd", companyId: row.companyId },
        issue: null,
        workspace: {
          baseCwd: row.cwd,
