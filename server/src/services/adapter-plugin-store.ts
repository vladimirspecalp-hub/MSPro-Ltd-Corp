 ...
 * ~/.mspro-ltd/adapter-plugins.json. This is the source of truth for which
...
export interface AdapterPluginRecord {
  /** npm package name (e.g., "droid-mspro-ltd-adapter") */
  packageName: string;
...
const MSPROLTD_DIR = path.join(os.homedir(), ".mspro-ltd");
const ADAPTER_PLUGINS_DIR = path.join(MSPROLTD_DIR, "adapter-plugins");
const ADAPTER_PLUGINS_STORE_PATH = path.join(MSPROLTD_DIR, "adapter-plugins.json");
const ADAPTER_SETTINGS_PATH = path.join(MSPROLTD_DIR, "adapter-settings.json");
...
function ensureDirs(): void {
  ...
  if (!fs.existsSync(pkgJsonPath)) {
    fs.writeFileSync(pkgJsonPath, JSON.stringify({
      name: "mspro-ltd-adapter-plugins",
      version: "0.0.0",
      private: true,
      description: "Managed directory for MSProLtd external adapter plugins. Do not edit manually.",
    }, null, 2) + "\n");
  }
}
