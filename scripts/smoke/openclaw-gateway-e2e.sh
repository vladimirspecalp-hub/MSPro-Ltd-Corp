...
MSPROLTD_API_URL="${MSPROLTD_API_URL:-http://127.0.0.1:3100}"
API_BASE="${MSPROLTD_API_URL%/}/api"
...
MSPROLTD_API_URL_FOR_OPENCLAW="${MSPROLTD_API_URL_FOR_OPENCLAW:-http://host.docker.internal:3100}"
...
AUTH_HEADERS=()
if [[ -n "${MSPROLTD_AUTH_HEADER:-}" ]]; then
  AUTH_HEADERS+=( -H "Authorization: ${MSPROLTD_AUTH_HEADER}" )
fi
if [[ -n "${MSPROLTD_COOKIE:-}" ]]; then
  AUTH_HEADERS+=( -H "Cookie: ${MSPROLTD_COOKIE}" )
  MSPROLTD_BROWSER_ORIGIN="${MSPROLTD_BROWSER_ORIGIN:-${MSPROLTD_API_URL%/}}"
  AUTH_HEADERS+=( -H "Origin: ${MSPROLTD_BROWSER_ORIGIN}" -H "Referer: ${MSPROLTD_BROWSER_ORIGIN}/" )
fi
...
require_board_auth() {
  if [[ ${#AUTH_HEADERS[@]} -eq 0 ]]; then
    fail "board auth required. Set MSPROLTD_COOKIE or MSPROLTD_AUTH_HEADER."
  fi
...
create_and_approve_gateway_join() {
  ...
  join_payload="$(jq -nc \
    ...
    --arg paperclipApiUrl "$MSPROLTD_API_URL_FOR_OPENCLAW" \
...
persist_claimed_key_artifacts() {
  ...
  local skill_dir="${OPENCLAW_CONFIG_DIR%/}/skills/mspro-ltd"
  local claimed_file="${workspace_dir}/mspro-ltd-claimed-api-key.json"
  local claimed_raw_file="${workspace_dir}/mspro-ltd-claimed-api-key.raw.json"
  ...
  if [[ -n "$container" ]]; then
    ...
    docker cp "$claimed_file" "${container}:/home/node/.openclaw/workspace/mspro-ltd-claimed-api-key.json" >/dev/null 2>&1 || true
    docker exec "$container" sh -lc "chmod 644 /home/node/.openclaw/workspace/mspro-ltd-claimed-api-key.json" >/dev/null 2>&1 || true
  fi

  if [[ "$AUTO_INSTALL_SKILL" == "1" ]]; then
    api_request "GET" "/skills/mspro-ltd"
    assert_status "200"
    {
      echo "MSPROLTD_API_URL=${MSPROLTD_API_URL_FOR_OPENCLAW}"
      echo
      printf "%s\n" "$RESPONSE_BODY"
    } > "${skill_dir}/SKILL.md"
...
inject_agent_api_key_payload_template() {
  ...
  local base_message
  base_message="Set MSPROLTD_API_KEY=${AGENT_API_KEY} in your run context before running MSProLtd heartbeat steps."
...
main() {
  ...
  wait_http_ready "${MSPROLTD_API_URL%/}/api/health" 15 || fail "MSProLtd API health endpoint not reachable"
  api_request "GET" "/health"
  assert_status "200"
  log "mspro-ltd health deploymentMode=$(jq -r '.deploymentMode // "unknown"' <<<"$RESPONSE_BODY") exposure=$(jq -r '.deploymentExposure // "unknown"' <<<"$RESPONSE_BODY")"
