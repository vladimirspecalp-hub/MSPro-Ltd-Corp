...
COMPANY_ID="${COMPANY_ID:-${MSPROLTD_COMPANY_ID:-}}"
...
SMOKE_IMAGE="${SMOKE_IMAGE:-mspro-ltd-openclaw-smoke:local}"
SMOKE_CONTAINER_NAME="${SMOKE_CONTAINER_NAME:-mspro-ltd-openclaw-smoke}"
...
Provide one of:
  MSPROLTD_AUTH_HEADER=\"Bearer <board-token>\"
  MSPROLTD_COOKIE=\"<board-session-cookie>\"
...
assert_status "200"
if ! grep -q "MSProLtd OpenClaw Gateway Onboarding" <<<"$RESPONSE_BODY"; then
...
for _ in $(seq 1 "$SMOKE_TIMEOUT_SEC"); do
  ...
  MATCH_COUNT="$(jq -r --arg agentId "$CREATED_AGENT_ID" '[.events[] | select(((.body.mspro-ltd.agentId // "") == $agentId))] | length' <<<"$LAST_EVENTS")"
