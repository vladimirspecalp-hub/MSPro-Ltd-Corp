...
base_cwd="${MSPROLTD_WORKSPACE_BASE_CWD:?MSPROLTD_WORKSPACE_BASE_CWD is required}"
worktree_cwd="${MSPROLTD_WORKSPACE_CWD:?MSPROLTD_WORKSPACE_CWD is required}"
paperclip_home="${MSPROLTD_HOME:-$HOME/.mspro-ltd}"
paperclip_instance_id="${MSPROLTD_INSTANCE_ID:-default}"
paperclip_dir="$worktree_cwd/.mspro-ltd"
worktree_config_path="$paperclip_dir/config.json"
worktree_env_path="$paperclip_dir/.env"
worktree_name="${MSPROLTD_WORKSPACE_BRANCH:-$(basename "$worktree_cwd")}"
...
source_config_path="${MSPROLTD_CONFIG:-}"
if [[ -z "$source_config_path" && ( -e "$base_cwd/.mspro-ltd/config.json" || -L "$base_cwd/.mspro-ltd/config.json" ) ]]; then
  source_config_path="$base_cwd/.mspro-ltd/config.json"
fi
...
run_paperclipai_command() {
  local command_args=("$@")
  if command -v pnpm >/dev/null 2>&1 && pnpm msproltdai --help >/dev/null 2>&1; then
    pnpm msproltdai "${command_args[@]}"
    return 0
  fi
  ...
  if command -v msproltdai >/dev/null 2>&1; then
    msproltdai "${command_args[@]}"
    return 0
  fi
...
write_fallback_worktree_config() {
  ...
  MSPROLTD_DIR="$paperclip_dir" \
  SOURCE_CONFIG_PATH="$source_config_path" \
  SOURCE_ENV_PATH="$source_env_path" \
  MSPROLTD_WORKTREES_DIR="${MSPROLTD_WORKTREES_DIR:-}" \
  node <<'EOF'
...
async function main() {
  ...
  const paperclipDir = process.env.MSPROLTD_DIR;
  const sourceConfigPath = process.env.SOURCE_CONFIG_PATH;
  const sourceEnvPath = process.env.SOURCE_ENV_PATH;
  const worktreeHome = path.resolve(expandHomePrefix(nonEmpty(process.env.MSPROLTD_WORKTREES_DIR) ?? "~/.mspro-ltd-worktrees"));
  ...
  const targetConfig = {
    ...
    storage: {
      ...
      s3: {
        bucket: sourceConfig?.storage?.s3?.bucket ?? "mspro-ltd",
  ...
  const inlineMasterKey = nonEmpty(sourceEnvEntries.MSPROLTD_SECRETS_MASTER_KEY);
  if (inlineMasterKey) {
    ...
    const sourceKeyFilePath = nonEmpty(sourceEnvEntries.MSPROLTD_SECRETS_MASTER_KEY_FILE)
      ? resolveRuntimeLikePath(sourceEnvEntries.MSPROLTD_SECRETS_MASTER_KEY_FILE, sourceConfigPath)
  ...
  const envLines = [
    "MSPROLTD_HOME=" + JSON.stringify(worktreeHome),
    "MSPROLTD_INSTANCE_ID=" + JSON.stringify(instanceId),
    "MSPROLTD_CONFIG=" + JSON.stringify(configPath),
    "MSPROLTD_CONTEXT=" + JSON.stringify(path.resolve(worktreeHome, "context.json")),
    "MSPROLTD_IN_WORKTREE=true",
    "MSPROLTD_WORKTREE_NAME=" + JSON.stringify(worktreeName),
  ];

  const agentJwtSecret = nonEmpty(sourceEnvEntries.MSPROLTD_AGENT_JWT_SECRET);
  if (agentJwtSecret) {
    envLines.push("MSPROLTD_AGENT_JWT_SECRET=" + JSON.stringify(agentJwtSecret));
  }
...
if paperclipai_command_available; then
  ...
  echo "msproltdai CLI not available in this workspace; writing isolated fallback config without DB seeding." >&2
  write_fallback_worktree_config
fi
...
if [[ -f "$worktree_cwd/package.json" && -f "$worktree_cwd/pnpm-lock.yaml" ]]; then
  ...
  if [[ "$needs_install" -eq 1 ]]; then
    backup_suffix=".mspro-ltd-backup-${BASHPID:-$$}"
    moved_symlink_paths=()
