...
const DEFAULT_QUERY = "\"Co-Authored-By: MSProLtd <noreply@mspro-ltd.ing>\"";
const DEFAULT_CACHE_FILE = path.resolve("data/mspro-ltd-commit-metrics-cache.json");
...
const MSPROLTD_EMAIL = "noreply@mspro-ltd.ing";
const MSPROLTD_NAME = "mspro-ltd";
...
function printHelp() {
  console.log(`Usage: tsx scripts/mspro-ltd-commit-metrics.ts [options]
...
function normalizeContributor(input: {
  ...
  if ((email && email === MSPROLTD_EMAIL) || (displayName && displayName.toLowerCase() === MSPROLTD_NAME)) {
    return null;
  }
...
function printSummary(summary: Summary) {
  console.log("MSProLtd commit metrics");
    ...
    while (true) {
      const response = await fetch(`${this.apiBase}${pathname}`, {
        headers: {
          ...
          "User-Agent": "mspro-ltd-commit-metrics",
