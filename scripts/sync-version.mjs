 ...
 *
 * Usage:
 *   node scripts/sync-version.mjs v0.1.5
 *   node scripts/sync-version.mjs 0.1.5
 *
 * Updates:
 *   - src-tauri/tauri.conf.json   (поле "version")
 *   - src-tauri/Cargo.toml        (поле version в [package])
 *   - package.json                (root)
 *   - server/package.json         (host server)
 *
 * Используется CI (.github/workflows/tauri-release.yml) перед сборкой,
 * чтобы все артефакты (MSI, host server, latest.json) имели одну версию,
 * соответствующую тегу. Закрывает D4 из P-2026-023 (host version sync).
 */
...
function parseVersion(input) {
  if (!input) {
    throw new Error("version argument is required (e.g. v0.1.5 or 0.1.5)");
  }
...
function updateCargoTomlVersion(relPath, version) {
  ...
  // Replace ONLY the version inside the [package] section (first version line after [package]).
...
function main() {
  ...
  const targets = [
    ...
    () => updateJsonVersion("server/package.json", version),
  ];
