#!/usr/bin/env bash
#
# Kill all local MSProLtd dev server processes (across all worktrees).
#
# Usage:
#   scripts/kill-dev.sh        # kill all mspro-ltd dev processes
...
while IFS= read -r line; do
  ...
  node_pids+=("$pid")
  node_lines+=("$line")
done < <(ps aux | grep -E '/mspro-ltd(-[^/]+)?/' | grep node | grep -v grep || true)
...
candidate_pidfiles=()
candidate_pidfiles+=(
  "$HOME"/.mspro-ltd/instances/*/db/postmaster.pid
  "$REPO_ROOT"/.mspro-ltd/instances/*/db/postmaster.pid
  "$REPO_ROOT"/.mspro-ltd/runtime-services/instances/*/db/postmaster.pid
)

for sibling_root in "$REPO_PARENT"/mspro-ltd*; do
  [[ -d "$sibling_root" ]] || continue
  candidate_pidfiles+=(
    "$sibling_root"/.mspro-ltd/instances/*/db/postmaster.pid
    "$sibling_root"/.mspro-ltd/runtime-services/instances/*/db/postmaster.pid
  )
done
...
if [[ ${#node_pids[@]} -eq 0 && ${#pg_pids[@]} -eq 0 && ${#browser_pids[@]} -eq 0 ]]; then
  echo "No MSProLtd dev processes found."
  exit 0
fi

if [[ ${#node_pids[@]} -gt 0 ]]; then
  echo "Found ${#node_pids[@]} MSProLtd dev node process(es):"
  echo ""
...
if [[ ${#node_pids[@]} -gt 0 ]]; then
  echo "Sending SIGTERM to MSProLtd node processes..."
