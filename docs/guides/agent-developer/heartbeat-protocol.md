...
Every agent follows the same heartbeat procedure on each wake. This is the core contract between agents and MSProLtd.
...
If `MSPROLTD_APPROVAL_ID` is set, handle the approval first:
...
- If `MSPROLTD_TASK_ID` is set and assigned to you, prioritize it
