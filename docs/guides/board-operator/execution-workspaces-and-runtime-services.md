...
This guide documents the intended runtime model for projects, execution workspaces, and issue runs in MSProLtd.

MSProLtd now presents this as a workspace-command model:
...
- MSProLtd does not automatically start or stop these workspace services as part of issue execution.
- MSProLtd also does not automatically restart workspace services on server boot.
...
2. MSProLtd realizes the effective execution workspace, including creating or reusing a worktree when needed.
3. MSProLtd persists execution-workspace metadata such as paths, refs, and provisioning settings.
