...
MSProLtd companies can be exported to portable markdown packages and imported from local directories or GitHub repositories. This lets you share company configurations, duplicate setups, and version-control your agent teams.
...
├── tasks/
│   └── onboarding/TASK.md
└── .mspro-ltd.yaml     # Adapter config, env inputs, routines
```
...
- **.mspro-ltd.yaml** holds MSProLtd-specific config (adapter types, env inputs, budgets) as an optional sidecar.
...
```sh
msproltdai company export <company-id> --out ./my-export
```
...
msproltdai company export abc123 --out ./backup --include company,agents,projects

# Export everything including tasks and skills
msproltdai company export abc123 --out ./full-export --include company,agents,projects,tasks,skills

# Export only specific skills
msproltdai company export abc123 --out ./skills-only --include skills --skills review,deploy
```
...
- Adapter type and env input declarations in `.mspro-ltd.yaml`
...
```sh
# From a local folder
msproltdai company import ./my-export

# From a GitHub URL
msproltdai company import https://github.com/org/repo

# From a GitHub subfolder
msproltdai company import https://github.com/org/repo/tree/main/companies/acme

# From GitHub shorthand
msproltdai company import org/repo
msproltdai company import org/repo/companies/acme
```
...
If `--target` is not specified, MSProLtd infers it: if a `--company-id` is provided (or one exists in context), it defaults to `existing`; otherwise `new`.
...
```sh
msproltdai company import org/repo --target existing --company-id abc123 --dry-run
```
...
```sh
msproltdai company import org/company-templates/engineering-team \
  --target new \
...
```sh
msproltdai company import ./shared-agents \
  --target existing \
  --company-id abc123 \
  --include agents \
  --collision rename
...
```sh
msproltdai company import org/repo --ref v2.0.0 --dry-run
```

**Non-interactive import (CI/scripts):**

```sh
msproltdai company import ./package \
  --target new \
  --yes \
  --json
```
...
MSProLtd supports several GitHub URL formats:
