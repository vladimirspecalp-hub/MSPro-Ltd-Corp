# ClipHub: Marketplace for MSProLtd Team Configurations
...
> The "app store" for whole-company AI teams — pre-built MSProLtd configurations, agent blueprints, skills, and governance templates that ship real work from day one.

## 1. Vision & Positioning

**ClipHub** sells **entire team configurations** — org charts, agent roles, inter-agent workflows, governance rules, and project templates — for MSProLtd-managed companies.

| Dimension | ClipHub |
|---|---|
| Unit of sale | Team blueprint (multi-agent org) |
| Buyer | Founder / team lead spinning up an AI company |
| Install target | MSProLtd company (agents, projects, governance) |
...
A complete MSProLtd company configuration:
...
Single-agent configurations designed to plug into a MSProLtd org:
...
Portable skill files that any MSProLtd agent can use:

- Markdown skill files with instructions
- Tool configurations and shell scripts
- Compatible with MSProLtd's skill loading system
...
interface Listing {
  ...
  paperclipVersionMin: string;     // Minimum MSProLtd version
...
interface Purchase {
  ...
  listingId: string;
  buyerUserId: string;
  buyerCompanyId: string | null;    // Target MSProLtd company
...
| `POST` | `/api/listings/:id/install` | Install to MSProLtd company |
...
Homepage → Browse marketplace → Filter by type/category
  ...
  → Select target MSProLtd company (or create new)
  → ClipHub API calls MSProLtd API to:
  ...
  → Redirect to MSProLtd dashboard with new team running
```
...
### 5.3 Creator: Export from MSProLtd → Publish

```
Running MSProLtd company → "Export as Blueprint" (CLI or UI)
  → MSProLtd exports:
...
- **Color palette**: Dark ink primary, warm sand backgrounds, accent color for CTAs (MSProLtd brand blue/purple)
...
| Install flow | One-click deploy to MSProLtd company |
...
{
  "targetCompanyId": "uuid",         // Existing MSProLtd company
...
3. For each agent in blueprint:
   - `POST /api/companies/:id/agents` (if `mspro-ltd-create-agent` supports it, or via approval flow)
...
- **Frontend**: Next.js (React), Tailwind CSS, same UI framework as MSProLtd
- **Backend**: Node.js API (or extend MSProLtd server)
- **Database**: Postgres (can share MSProLtd's DB or separate)
- **Payments**: Stripe Connect (marketplace mode)
- **Storage**: S3/R2 for listing bundles and images
- **Auth**: Shared with MSProLtd auth (or OAuth2)

### 9.2 Integration with MSProLtd

ClipHub can be:
- **Option A**: A separate app that calls MSProLtd's API to install blueprints
- **Option B**: A built-in section of the MSProLtd UI (`/marketplace` route)

Option B is simpler for MVP — adds routes to the existing MSProLtd UI and API.
...
- [ ] Install flow: blueprint → MSProLtd company
...
- [ ] "Export from MSProLtd" CLI command
