...
Fires a webhook trigger from an external system. Requires a valid `Authorization` or `X-MSProLtd-Signature` + `X-MSProLtd-Timestamp` header pair matching the trigger's signing mode.
