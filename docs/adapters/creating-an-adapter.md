...
Build a custom adapter to connect MSProLtd to any agent runtime.
...
|---|---|---|
| Source | Inside `mspro-ltd-fork` | Separate npm package |
| Distribution | Ships with MSProLtd | Independent npm publish |
...
For most cases, **build an external adapter plugin**. It's cleaner, independently versioned, and doesn't require modifying MSProLtd's source. See [External Adapters](/adapters/external-adapters) for the full guide.
...
1. Read config using safe helpers (`asString`, `asNumber`, etc.) from `@msproltd/adapter-utils/server-utils`
...
|--------|--------|---------|
| `runChildProcess(cmd, opts)` | `@msproltd/adapter-utils/server-utils` | Spawn with timeout, grace, streaming |
| `buildPaperclipEnv(agent)` | `@msproltd/adapter-utils/server-utils` | Inject `MSPROLTD_*` env vars |
| `renderTemplate(tpl, data)` | `@msproltd/adapter-utils/server-utils` | `{{variable}}` substitution |
| `asString(v)` | `@msproltd/adapter-utils` | Safe config value extraction |
| `asNumber(v)` | `@msproltd/adapter-utils` | Safe number extraction |
...
For built-in adapters registered in MSProLtd's source:
...
`format-event.ts` — pretty-prints stdout for `msproltdai run --watch` using `picocolors`.
...
With these flags set, the MSProLtd UI will automatically show the instructions bundle editor, skills management tab, and working directory field for agents using this adapter — no MSProLtd source changes required.
...
Make MSProLtd skills discoverable to your agent runtime without writing to the agent's working directory:
