...
summary: What adapters are and how they connect agents to MSProLtd
---

Adapters are the bridge between MSProLtd's orchestration layer and agent runtimes. Each adapter knows how to invoke a specific type of AI agent and capture its results.

## How Adapters Work

When a heartbeat fires, MSProLtd:
...
| Hermes Local | `hermes_local` | Runs Hermes CLI locally (`hermes-mspro-ltd-adapter`) |
...
|---------|---------|----------|-------------|
| Droid Local | `@henkey/droid-mspro-ltd-adapter` | `droid_local` | Runs Factory Droid locally |

## External Adapters

You can build and distribute adapters as standalone packages — no changes to MSProLtd's source code required. External adapters are loaded at startup via the plugin system.
...
my-adapter/
  src/
    ...
    cli/
      format-event.ts   # Terminal output for `msproltdai run --watch`
```
...
External adapters can ship a self-contained UI parser that tells the MSProLtd web UI how to render their stdout. Without it, the UI uses a generic shell parser. See the [UI Parser Contract](/adapters/adapter-ui-parser) for details.
