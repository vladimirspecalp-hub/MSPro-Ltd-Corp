...
This specification is vendor-neutral. It is intended to be usable by any agent-company runtime, not only MSProLtd.
...
- vendors may attach the actual schedule / trigger / runtime fidelity in a vendor extension such as `.mspro-ltd.yaml`
...
- MSProLtd must not require extra top-level fields for skill validity
- MSProLtd-specific extensions must live under `metadata.mspro-ltd` or `metadata.sources`
...
For MSProLtd, the preferred fidelity extension is:
...
- issue/task MSProLtd-only metadata
...
- MSProLtd tools may emit the vendor extension by default as a sidecar while keeping the base markdown clean

Suggested MSProLtd shape:
...
Additional rules for MSProLtd exporters:
...
- omit empty and default-valued MSProLtd fields when possible
...
- MSProLtd exporters should emit `.mspro-ltd.yaml` as a sidecar by default
...
MSProLtd can map this spec to its runtime model like this:
...
- MSProLtd extension:
  - `.mspro-ltd.yaml` -> adapter config, runtime config, env input declarations, permissions, budgets, routine triggers, and other MSProLtd-specific fidelity

Inline MSProLtd-only metadata that must live inside a shared markdown file should use:

- `metadata.mspro-ltd`

That keeps the base format broader than MSProLtd.

This specification itself remains vendor-neutral and intended for any agent-company runtime, not only MSProLtd.

## 20. Cutover

MSProLtd should cut over to this markdown-first package model as the primary portability format.

`mspro-ltd.manifest.json` does not need to be preserved as a compatibility requirement for the future package system.

For MSProLtd, this should be treated as a hard cutover in product direction rather than a long-lived dual-format strategy.
