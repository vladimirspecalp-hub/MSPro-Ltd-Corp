...
// MSPRO fork: subscription-only (Cursor Pro plan)
function resolveCursorBillingType(_env: Record<string, string>): "subscription" {
  return "subscription";
}

function resolveCursorBiller(
  _env: Record<string, string>,
  _billingType: "subscription",
  _provider: string | null,
): string {
  return "cursor";
}

const CURSOR_FORBIDDEN_API_ENV_VARS = [
  "CURSOR_API_KEY",
  "OPENAI_API_KEY",
  "ANTHROPIC_API_KEY",
  "OPENROUTER_API_KEY",
...
function guardCursorSubscriptionOnly(env: Record<string, string>): void {
  const found: string[] = [];
  for (const key of CURSOR_FORBIDDEN_API_ENV_VARS) {
...
export async function execute(ctx: AdapterExecutionContext): Promise<AdapterExecutionResult> {
  ...
  // MSPRO fork: жёсткая политика — падаем если API-keys прокинуты
  guardCursorSubscriptionOnly(effectiveEnv);
