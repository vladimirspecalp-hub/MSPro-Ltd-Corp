...
// MSPRO fork: Bedrock-ветка убрана, isBedrockModelId больше не используется
...
export async function testEnvironment(
  ...
  // MSPRO fork: subscription-only. API-ключи и Bedrock запрещены — поднимаем ERROR а не WARN.
  ...
  if (hasBedrock) {
    checks.push({
      code: "claude_bedrock_forbidden_mspro_fork",
      level: "error",
      message: "AWS Bedrock detected. This MSPRO fork supports subscription-only — Bedrock is disabled.",
      hint: "Unset CLAUDE_CODE_USE_BEDROCK and ANTHROPIC_BEDROCK_BASE_URL everywhere.",
    });
  }
  if (isNonEmpty(configApiKey) || isNonEmpty(hostApiKey)) {
    const source = isNonEmpty(configApiKey) ? "adapter config env" : "server environment";
    checks.push({
      code: "claude_api_key_forbidden_mspro_fork",
      level: "error",
      message:
        "ANTHROPIC_API_KEY detected. This MSPRO fork runs exclusively via Claude Max subscription — API keys are blocked.",
      detail: `Detected in ${source}.`,
      hint: "Remove ANTHROPIC_API_KEY from both shell env and adapterConfig.env. Login via `claude` CLI (subscription) instead.",
    });
  }
  if (!hasBedrock && !isNonEmpty(configApiKey) && !isNonEmpty(hostApiKey)) {
    checks.push({
      code: "claude_subscription_mode_ok",
      level: "info",
      message: "Subscription mode active. Ensure you've run `claude` CLI once and logged in.",
    });
  }
  ...
  if (canRunProbe) {
    if (!commandLooksLike(command, "claude")) {
      ...
      // MSPRO fork: subscription-only, всегда передаём --model если задан
      if (model) {
