/**
 * Core types for the MSProLtd plugin worker-side SDK.
 *
 * These types define the stable public API surface that plugin workers import
 * from `@msproltd/plugin-sdk`.  The host provides a concrete implementation
...
// ---------------------------------------------------------------------------
// Re-exports from @msproltd/shared (plugin authors import from one place)
// ---------------------------------------------------------------------------
...
export interface ScopeKey {
  /** What kind of MSProLtd object this state is scoped to. */
...
export interface PluginEntityUpsert {
  ...
  externalId?: string;
  /** Human-readable title for display in the MSProLtd UI. */
  title?: string;
...
/**
 * `ctx.events` — subscribe to and emit MSProLtd domain events.
 *
...
export interface PluginEventsClient {
  /**
   * Subscribe to a core MSProLtd domain event or a plugin-namespaced event.
   *
 ...
 * This client resolves the reference through the MSProLtd secret provider
...
export interface PluginSecretsClient {
   ...
   * in the MSProLtd secret provider (e.g. `"MY_API_KEY"`).
   *
 ...
 * ```
 *
 * @see usePluginStream in `@msproltd/plugin-sdk/ui`
 */
