...
type PaperclipSprite = (typeof MSPROLTD_SPRITES)[number];
...
export function AsciiArtAnimation() {
  ...
  useEffect(() => {
    ...
    function drawStaticFrame() {
      ...
      for (let baseRow = 1; baseRow < rows - 9; baseRow += gapY) {
        ...
        for (let baseCol = startX; baseCol < cols - 10; baseCol += gapX) {
          const sprite = MSPROLTD_SPRITES[(baseCol + baseRow) % MSPROLTD_SPRITES.length]!;
    ...
    function spawnClip() {
      const sprite = MSPROLTD_SPRITES[Math.floor(Math.random() * MSPROLTD_SPRITES.length)]!;
