...
For `claude_local` and `codex_local`, MSProLtd assumes the CLI is already installed and authenticated on the host machine.
...
MSProLtd stores resumable session state per `(agent, taskKey, adapterType)`.
