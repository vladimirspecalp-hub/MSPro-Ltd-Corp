...
This document explains how MSProLtd interprets issue assignment, issue status, execution runs, wakeups, parent/sub-issue structure, and blocker relationships.

`doc/SPEC-implementation.md` remains the V1 contract. This document is the detailed execution model behind that contract.

## 1. Core Model

MSProLtd separates four concepts that are easy to blur together:
...
This is a hard invariant. MSProLtd is single-assignee by design.

## 3. Status Semantics

MSProLtd issue statuses are not just UI labels. They imply different expectations about ownership and execution.
...
- for agent-assigned work, MSProLtd may still need a wake path to ensure the assignee actually sees it
...
- MSProLtd can wake the assignee
- MSProLtd can track runs linked to the issue
- MSProLtd can recover some lost execution state after crashes/restarts

### User-owned issues

User-owned issues are not executed by the heartbeat scheduler.

- MSProLtd can track the ownership and status
- MSProLtd cannot rely on heartbeat/run semantics to keep them moving
...
MSProLtd already clears stale execution locks and can adopt some stale checkout locks when the original run is gone.

## 6. Parent/Sub-Issue vs Blockers

MSProLtd uses two different relationships for different jobs.
...
For agent-assigned, non-terminal, actionable issues, MSProLtd should not leave work in a state where nobody is working it and nothing will wake it.
...
MSProLtd now treats crash/restart recovery as a stranded-assigned-work problem, not just a stranded-run problem.
...
- if the latest issue-linked run failed/timed out/cancelled and no live execution path remains, MSProLtd queues one automatic assignment recovery wake
- if that recovery wake also finishes and the issue is still stranded, MSProLtd moves the issue to `blocked` and posts a visible comment
...
- MSProLtd queues one automatic continuation wake
- if that continuation wake also finishes and the issue is still stranded, MSProLtd moves the issue to `blocked` and posts a visible comment
...
On startup and on the periodic recovery loop, MSProLtd now does three things in sequence:
...
That is the execution contract MSProLtd should present to operators.
