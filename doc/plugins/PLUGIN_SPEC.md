# MSProLtd Plugin System Specification

Status: proposed complete spec for the post-V1 plugin system

This document is the complete specification for MSProLtd's plugin and extension architecture.
...
- Plugin UI bundles currently run as same-origin JavaScript inside the main MSProLtd app. Treat plugin UI as trusted code, not a sandboxed frontend capability boundary.
- Manifest capabilities currently gate worker-side host RPC calls. They do not prevent plugin UI code from calling ordinary MSProLtd HTTP APIs directly.
...
MSProLtd plugin design is based on the following assumptions:

1. MSProLtd is single-tenant and self-hosted.
2. Plugin installation is global to the instance.
3. "Companies" remain core MSProLtd business objects, but they are not plugin trust boundaries.
4. Board governance, approval gates, budget hard-stops, and core task invariants remain owned by MSProLtd core.
...
2. Let plugins add major capabilities without editing MSProLtd core.
...
4. Depend on project-local plugin folders such as `.mspro-ltd/plugins`.
...
The single MSProLtd deployment an operator installs and controls.

### 5.2 Company

A first-class MSProLtd business object inside the instance.
...
A trusted in-process extension loaded directly by MSProLtd core.
...
An installable instance-wide extension package loaded through the MSProLtd plugin runtime.
...
MSProLtd has two extension classes.
...
MSProLtd already has a concrete workspace model:
...
Plugins live under the MSProLtd instance directory.

Suggested layout:

- `~/.mspro-ltd/instances/default/plugins/package.json`
- `~/.mspro-ltd/instances/default/plugins/node_modules/`
- `~/.mspro-ltd/instances/default/plugins/.cache/`
- `~/.mspro-ltd/instances/default/data/plugins/<plugin-id>/`
...
MSProLtd should add CLI commands:

- `pnpm msproltdai plugin list`
- `pnpm msproltdai plugin install <package[@version]>`
- `pnpm msproltdai plugin uninstall <plugin-id>`
- `pnpm msproltdai plugin upgrade <plugin-id> [version]`
- `pnpm msproltdai plugin doctor <plugin-id>`
...
- UI slot IDs are automatically namespaced by plugin ID (e.g. `@mspro-ltd/plugin-linear:sync-health-widget`), so cross-plugin collisions are structurally impossible
...
Plugins may contribute tools that MSProLtd agents can use during runs.
...
- MSProLtd server starts one worker process per installed plugin
...
Receives one typed MSProLtd domain event.
...
```tsx
// dist/ui/index.tsx
import { usePluginData, usePluginAction, MetricCard, StatusBadge } from "@msproltd/plugin-sdk/ui";
...
- The host provides **design tokens and shared components** via `@msproltd/plugin-sdk/ui` so plugins can match the host's visual language without being forced to.
...
### 19.0.1 Plugin UI SDK (`@msproltd/plugin-sdk/ui`)
...
- Plugin bundles must not import from host internals. They may only import from `@msproltd/plugin-sdk/ui` and their own dependencies.
...
## 19.6 Shared Components In `@msproltd/plugin-sdk/ui`
...
The `@msproltd/plugin-sdk/ui` subpath should also export an `ErrorBoundary` component that plugin authors can use to catch rendering errors without crashing the host page.
...
- secret ref fields: any schema property annotated with `"format": "secret-ref"` renders as a secret picker that resolves through the MSProLtd secret provider system rather than a plain text input
...
1. Core MSProLtd data stays in first-party tables.
2. Most plugin-owned data starts in generic extension tables.
3. Plugin data should scope to existing MSProLtd objects before new tables are introduced.
4. Arbitrary third-party schema migrations are out of scope for the first plugin system.

## 21.2 Core Table Reuse

If data becomes part of the actual MSProLtd product model, it should become a first-party table.

Examples:

- `project_workspaces` is already first-party
- if MSProLtd later decides git state is core product data, it should become a first-party table too
...
2. Secret refs resolve through the existing MSProLtd secret provider system.
...
6. The operator may force-purge immediately via CLI: `pnpm msproltdai plugin purge <plugin-id>`.
...
Plugin install, uninstall, upgrade, and config changes **must** take effect without restarting the MSProLtd server. This is a normative requirement, not optional.
...
### 27.1 `@msproltd/plugin-test-harness`
...
```ts
import { createTestHarness } from "@msproltd/plugin-test-harness";
...
For developing a plugin against a running MSProLtd instance:

- The operator installs the plugin from a local path: `pnpm msproltdai plugin install ./path/to/plugin`
...
The host should publish a starter template (`create-mspro-ltd-plugin`) that scaffolds:
...
- `@mspro-ltd/plugin-workspace-files`
- `@mspro-ltd/plugin-terminal`
- `@mspro-ltd/plugin-git`
- `@mspro-ltd/plugin-linear`
- `@mspro-ltd/plugin-github-issues`
- `@mspro-ltd/plugin-grafana`
- `@mspro-ltd/plugin-runtime-processes`
- `@mspro-ltd/plugin-stripe`
...
- `@msproltd/plugin-sdk` — the complete plugin SDK

The package uses subpath exports to separate worker and UI concerns:

- `@msproltd/plugin-sdk` — worker-side SDK (context, events, state, tools, logger, `definePlugin`, `z`)
- `@msproltd/plugin-sdk/ui` — frontend SDK (bridge hooks, shared components, design tokens)
...
2. **Tied to API version**: Each major SDK version corresponds to exactly one plugin `apiVersion`. When `@msproltd/plugin-sdk@2.x` ships, it targets `apiVersion: 2`. Plugins built with SDK 1.x continue to declare `apiVersion: 1`.
...
1. Plugin author updates `@msproltd/plugin-sdk` dependency.
...
- webhooks
- settings page
- plugin UI bundle loading, host bridge, and `@msproltd/plugin-sdk/ui`
...
- `@msproltd/plugin-test-harness`
- `create-mspro-ltd-plugin` starter template
...
MSProLtd should not implement a generic in-process hook bag modeled directly after local coding tools.

MSProLtd should implement:
...
That is the complete target design for the MSProLtd plugin system.
