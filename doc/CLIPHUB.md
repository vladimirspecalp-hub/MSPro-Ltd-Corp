...
ClipHub is the public registry where people share, discover, and download MSProLtd company configurations. A company template is a portable artifact containing an entire org — agents, reporting structure, adapter configs, role definitions, seed tasks — ready to spin up with one command.

---

## What It Is

ClipHub is to MSProLtd what a package registry is to a programming language. MSProLtd already supports exportable org configs (see [SPEC.md](./SPEC.md) §2). ClipHub is the public directory where those exports live.

A user builds a working company in MSProLtd — a dev shop, a marketing agency, a research lab, a content studio — exports the template, and publishes it to ClipHub. Anyone can browse, search, download, and spin up that company on their own MSProLtd instance.
...
A ClipHub package is a **company template export** — the portable artifact format defined in the MSProLtd spec. It contains:
...
**Install (fresh start):**
```
mspro-ltd install cliphub:<publisher>/<company-slug>
```
Downloads the template and creates a new company in your local MSProLtd instance. You add your own API keys, set budgets, customize agents, and hit go.
...
From within MSProLtd, export your company as a template, then publish:

```
mspro-ltd export --template my-company
mspro-ltd publish cliphub my-company
```
...
ClipHub is a **separate service** from MSProLtd itself. MSProLtd is self-hosted; ClipHub is a hosted registry that MSProLtd instances talk to.
...
| **MSProLtd CLI** | `msproltdai install`, `msproltdai publish`, `msproltdai cliphub sync` — built into MSProLtd |
| **MSProLtd UI** | "Browse ClipHub" panel in the MSProLtd web UI for discovering templates without leaving the app |

### Tech Stack

| Layer | Technology |
|---|---|
| Frontend | React + Vite (consistent with MSProLtd) |
| Backend | TypeScript + Hono (consistent with MSProLtd) |
| Database | PostgreSQL |
...
4. Run `msproltdai install cliphub:acme/lean-saas-shop`
5. MSProLtd creates the company locally with all agents pre-configured
6. Set your API keys, adjust budgets, add your initial tasks
7. Hit go

### "I built something great and want to share it"

1. Build and iterate on a company in MSProLtd until it works well
2. Export: `msproltdai export --template my-agency`
3. Publish: `msproltdai publish cliphub my-agency`
...
3. Install just that agent: `msproltdai install cliphub:acme/senior-python-eng --agent`
4. Assign it to a manager in your existing company
5. Done

---

## Relationship to MSProLtd

ClipHub is **not required** to use MSProLtd. You can build companies entirely from scratch without ever touching ClipHub. But ClipHub dramatically lowers the barrier to entry:
...
ClipHub is to MSProLtd what a package registry is to a language runtime: optional, but transformative.
...
- [ ] `msproltdai install cliphub:<publisher>/<slug>` CLI command
...
- [ ] "Browse ClipHub" panel in MSProLtd web UI
- [ ] `msproltdai cliphub sync` for bulk publishing
...
- Running companies on ClipHub (it's a registry, not a runtime — consistent with MSProLtd's own philosophy)
