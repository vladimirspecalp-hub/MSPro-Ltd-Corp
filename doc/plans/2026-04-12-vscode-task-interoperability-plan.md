...
MSProLtd should not replace its workspace runtime service model with VS Code tasks.
...
- MSProLtd owns long-running workspace services and their desired state
- MSProLtd shows operators exactly which named thing they are starting or stopping
- MSProLtd distinguishes long-running services from one-shot jobs
...
This is directionally correct for MSProLtd because it gives the control plane an explicit model for service lifecycle, health, reuse, and restart behavior.
...
The issue is that the configuration surface is too low-level and MSProLtd does not yet leverage existing workspace metadata.

## Recommendation

Keep MSProLtd runtime services as the source of truth for service supervision.
...
4. `Runtime service instance`
   The live process record that already exists today in MSProLtd.

In that model, VS Code tasks are a way to populate workspace commands.
Only commands that map cleanly to MSProLtd service or job semantics should become runnable in MSProLtd.

## Why Not Fully Adopt VS Code Tasks

VS Code tasks are broader than MSProLtd runtime services.
...
That creates a bad fit if MSProLtd tries to use `tasks.json` as its only runtime model:
...
- problem matcher readiness is useful metadata, but it is not enough to replace MSProLtd's persisted service lifecycle model

The right boundary is interoperability, not replacement.

## Interoperability Contract

MSProLtd should support a conservative subset of VS Code tasks and clearly mark unsupported entries.

### Supported in phase 1

- `shell` and `process` tasks with a concrete command MSProLtd can resolve
...
- selected variable substitution when MSProLtd can resolve it safely from workspace context
- mapping task metadata into MSProLtd readiness/expose hints
- limited compound-task launch flows

### Not supported initially

- extension-provided task types MSProLtd cannot execute directly
...
MSProLtd needs an explicit classification layer instead of assuming every VS Code task is a service.

Recommended classification:

- `service`
  Explicitly marked by MSProLtd metadata, or confidently inferred from background/watch task semantics

- `job`
  One-shot command expected to exit

- `unsupported`
  Present in `tasks.json`, but not safely runnable by MSProLtd
...
For a workspace with `cwd`, MSProLtd should look for `.vscode/tasks.json`.
...
MSProLtd should map imported tasks with explicit, documented rules.

Recommended rules:

1. A task becomes a `job` by default.
2. A task becomes a `service` only when:
   - MSProLtd metadata marks it as a service, or
...
MSProLtd-specific metadata can live in a namespaced field on the imported task definition, for example:
...
Success condition:
an operator can see what MSProLtd would import and why.
...
Goal:
run supported imported service commands through the current MSProLtd supervisor.
...
Success condition:
imported service commands behave like native MSProLtd services once adopted.
...
- removal of MSProLtd's own runtime supervision model
- editor-dependent execution semantics inside the control plane

## Risks

- overfitting MSProLtd to VS Code and making the model worse for non-VS-Code repos
...
MSProLtd should adopt VS Code tasks as an optional workspace command source, not as the canonical runtime model.
...
- Existing MSProLtd workspace plan: `doc/plans/2026-03-10-workspace-strategy-and-git-worktrees.md`
