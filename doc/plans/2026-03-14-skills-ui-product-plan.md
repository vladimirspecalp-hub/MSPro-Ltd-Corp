...
This document defines the product and UI plan for skill management in MSProLtd.
...
- MSProLtd company import/export can include skills as package content
...
3. The UI must always tell the truth about what MSProLtd knows:
   - desired state in MSProLtd
...
MSProLtd should treat skills at two scopes:
...
- created locally inside MSProLtd later
...
7. “Understand what MSProLtd knows vs what the adapter knows.”
...
- importing from a `skills.sh`-compatible source should work without requiring a MSProLtd-specific package layout
...
This matters because Codex and similar adapters may already have local skills that MSProLtd did not install.

These should be clearly marked:

- external
- not managed by MSProLtd
...
- `desired_only`: MSProLtd wants it, adapter does not show it yet
- `external`: adapter has it but MSProLtd does not manage it
- `drifted`: adapter has a conflicting or unexpected version/location
- `unmanaged`: adapter does not support sync, MSProLtd only tracks desired state
...
- MSProLtd can still track desired skills
...
- MSProLtd can see adapter skills
...
- `.mspro-ltd.yaml` may add MSProLtd-specific fidelity, but should not replace the base shortname association model
...
- `.mspro-ltd.yaml` may add MSProLtd-specific skill fidelity later if needed, but should not be required for ordinary agent-to-skill association
...
MSProLtd should have a company-scoped skill model or managed package model representing:
...
MSProLtd should separately store:
...
5. If agent skill associations are path-based instead of shortname-based, the format will feel too technical and too MSProLtd-specific.
...
That gives MSProLtd one coherent skill story instead of forcing package management, adapter sync, and agent configuration into the same screen.
