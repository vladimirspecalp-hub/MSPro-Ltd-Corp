...
Agents need API keys to authenticate with MSProLtd. The current approach
...
**Trust model:** The adapter process runs on the same machine as the MSProLtd
server (or is invoked directly by it). There is no meaningful network boundary.

**Approach:** MSProLtd generates a token and passes it directly to the agent
...
`MSPROLTD_API_URL`, `MSPROLTD_AGENT_ID`, `MSPROLTD_COMPANY_ID`. We need to
add a `MSPROLTD_API_KEY` (JWT) to the set of injected env vars.
...
**Approach:** Similar to `claude setup-token` -- the developer runs a MSProLtd CLI
...
```
mspro-ltd auth login
# Opens browser -> user confirms -> token stored at ~/.mspro-ltd/credentials
```
...
managed by the MSProLtd server itself.
...
1. A company admin (user or agent) generates an **invite URL** from MSProLtd.
...
3. The agent fetches the URL, which returns an **onboarding document**
   ...
   - The MSProLtd SKILL.md (or a link to it)
   - What information MSProLtd needs from the agent (e.g. webhook URL, adapter
...
5. MSProLtd stores the pending registration.
...
7. On approval, MSProLtd provisions the agent's credentials and sends the
   first heartbeat.

**Token format:** MSProLtd issues an API key (or JWT) upon approval, delivered
...
```json
{
  ...
  "onboarding": {
    ...
    "skillUrl": "https://app.mspro-ltd.ing/skills/mspro-ltd/SKILL.md",
    "requiredFields": {
      "name": "Your display name",
      "adapterType": "How MSProLtd should send you heartbeats",
...
1. Generate a MSProLtd invite link for the company.
...
4. A MSProLtd company member approves the new agent.
5. MSProLtd begins sending heartbeats to the OpenClaw webhook endpoint.
...
| Priority | Item                              | Notes                                                                                            |
| -------- | --------------------------------- | ------------------------------------------------------------------------------------------------ |
| **P0**   | Local adapter JWT injection       | Unblocks zero-config local auth. Mint a JWT per heartbeat, pass as `MSPROLTD_API_KEY`.          |
...
| **P3**   | CLI auth flow                     | `msproltdai auth login` for developer-managed remote agents.                                      |
