...
`PAP-447` asks how MSProLtd should support worktree-driven coding workflows for local coding agents without turning that into a universal product requirement.
...
- the workflow should reuse the same MSProLtd instance and embedded database instead of creating a blank environment
- local agent auth should remain low-friction

At the same time, we do not want to hard-code "every agent uses git worktrees" into MSProLtd:

- some operators use MSProLtd to manage MSProLtd and want worktrees heavily
- other operators will not want worktrees at all
- not every adapter runs in a local git repository
- not every adapter runs on the same machine as MSProLtd
- Claude and Codex expose different built-in affordances, so MSProLtd should not overfit to one tool

## Core Product Decision

MSProLtd should model **execution workspaces**, not **worktrees**.
...
- local adapters may receive a real filesystem cwd produced by MSProLtd
...
- MSProLtd should not assume that every adapter can see or use a host filesystem path directly
...
If MSProLtd makes worktrees agent-first, it will blur:
...
- MSProLtd core resolves and records execution workspace state
...
That gives MSProLtd room to support:
...
- MSProLtd handles workspace runtime services under the hood using project/workspace policy
...
Once MSProLtd is creating isolated issue checkouts, it is implicitly touching a bigger workflow:
...
So MSProLtd should distinguish:
...
- only add adapter-specific runtime UI when an adapter truly needs settings that MSProLtd cannot infer
...
- when issue moves to done, should MSProLtd auto-commit?
- should it auto-push?
...
MSProLtd already has the right foundation for a project-first model.
...
For server-spawned local adapters, MSProLtd already injects a short-lived local JWT:
...
- Codex CLI does not present a single first-class portable CLI worktree abstraction that MSProLtd should mirror directly

Implication:

- for `codex_local`, MSProLtd should usually create/select the checkout itself and then launch Codex inside that cwd
...
- but MSProLtd should still treat that as an adapter optimization, not the canonical cross-adapter model
...
Because of that, MSProLtd should separate:
...
MSProLtd should be able to express intentions such as:
...
These are not specific to MSProLtd. They are a common property of working in a dev workspace, whether local or remote.
...
```json
{
  "workspaceStrategy": {
    ...
    "worktreeParentDir": ".mspro-ltd/instances/default/worktrees/projects/{{project.id}}",
...
Do not model this as a MSProLtd-specific `devServer` flag.
...
MSProLtd should distinguish between:
...
MSProLtd should normalize the reported metadata without requiring every adapter to look like a host-local process.
...
In addition to the local git helper, MSProLtd should define a generic runtime service manager contract.
...
- remote sandbox adapter: create its own isolated workspace from repo/ref/branch intent and report the realized remote workspace metadata back to MSProLtd
...
For runtime services specifically, MSProLtd should eventually track normalized fields such as:
...
- runtime services are expressed generically, not as MSProLtd-only dev-server flags
...
- same embedded DB / MSProLtd instance can be reused through env/config injection when appropriate
...
- MSProLtd can decide whether to reuse or start a fresh service deterministically
...
3. Test against MSProLtd-on-MSProLtd workflows first.
...
7. Workspace runtime services are modeled generically, not as MSProLtd-specific dev-server toggles.
