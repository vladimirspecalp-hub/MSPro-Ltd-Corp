...
This document defines a V1 plan for "smart model routing" in MSProLtd.
...
- preserve MSProLtd's existing task, session, and audit invariants
...
That separation is a better fit for MSProLtd than copying Hermes' exact keyword heuristic.

## 3. Current MSProLtd State

MSProLtd already has the right execution shape for adapter-specific routing, but it currently assumes one model per heartbeat run.
...
MSProLtd should implement smart model routing as an adapter-local, opt-in execution pattern.

V1 decision:

1. Do not add a global server-side router that tries to understand every adapter.
2. Do not copy Hermes' prompt-keyword classifier as MSProLtd's default routing policy.
...
- MSProLtd heartbeats are structured, issue-scoped, and already include wake metadata
...
- the adapter already injects MSProLtd skills and workspace metadata cleanly
...
This matters because MSProLtd's board UI is supposed to make cost and behavior legible.

## 9. Why Not Copy Hermes Exactly

Hermes' cheap-route heuristic is useful precedent, but MSProLtd should not start there.

Reasons:

- Hermes is optimizing free-form conversational turns
- MSProLtd agents run structured, issue-scoped heartbeats with explicit task and workspace context
- MSProLtd already knows whether a run is fresh vs resumed, issue-scoped vs approval follow-up, and what workspace/session exists
- those execution facts are stronger routing signals than prompt keyword matching

If MSProLtd later wants a cheap-only completion path for trivial runs, that can be a second-stage feature built on observed run data, not the first implementation.
...
MSProLtd should ship smart model routing as:
