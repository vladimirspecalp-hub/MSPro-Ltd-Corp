...
- New config/env settings:
  - `MSPROLTD_AGENT_JWT_SECRET`
  - `MSPROLTD_AGENT_JWT_TTL_SECONDS` (default: `172800`)
  - `MSPROLTD_AGENT_JWT_ISSUER` (default: `mspro-ltd`)
  - `MSPROLTD_AGENT_JWT_AUDIENCE` (default: `mspro-ltd-api`)
...
1. In:
   ...
   inject `MSPROLTD_API_KEY` from context token.

- Preserve existing behavior for explicit user-defined env vars in `adapterConfig.env`:
  - if user already sets `MSPROLTD_API_KEY`, do not overwrite it.
- Continue injecting:
  - `MSPROLTD_AGENT_ID`
  - `MSPROLTD_COMPANY_ID`
  - `MSPROLTD_API_URL`
...
- Local adapters authenticate without manual `MSPROLTD_API_KEY` config.
