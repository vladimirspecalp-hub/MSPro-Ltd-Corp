...
description: >
  Technical guide for creating a new MSProLtd agent adapter. Use when building
...
# Creating a MSProLtd Agent Adapter

An adapter bridges MSProLtd's orchestration layer to a specific AI agent runtime (Claude Code, Codex CLI, a custom process, an HTTP endpoint, etc.). Each adapter is a self-contained package that provides implementations for **three consumers**: the server, the UI, and the CLI.
...
packages/adapters/<name>/
  src/
    ...
    cli/
      ...
      format-event.ts   # Colored terminal output for `msproltdai run --watch`
  package.json
  tsconfig.json
```
...
## 2. Shared Types (`@msproltd/adapter-utils`)

All adapter interfaces live in `packages/adapter-utils/src/types.ts`. Import from `@msproltd/adapter-utils` (types) or `@msproltd/adapter-utils/server-utils` (runtime helpers).
...
interface AdapterExecutionResult {
  ...
  clearSession?: boolean;         // true = tell MSProLtd to forget the stored session
}
...
```json
{
  ...
  "dependencies": {
    "@msproltd/adapter-utils": "workspace:*",
    "picocolors": "^1.1.1"
  },
  "devDependencies": {
    "typescript": "^5.7.3"
...
The `agentConfigurationDoc` is read by LLM agents (including MSProLtd agents that create other agents). Write it as **routing logic**, not marketing copy. Include concrete "use when" and "don't use when" guidance so an LLM can decide whether this adapter is appropriate for a given task.
...
1. **Read config** — extract typed values from `ctx.config` using helpers (`asString`, `asNumber`, `asBoolean`, `asStringArray`, `parseObject` from `@msproltd/adapter-utils/server-utils`)
2. **Build environment** — call `buildPaperclipEnv(agent)` then layer in `MSPROLTD_RUN_ID`, context vars (`MSPROLTD_TASK_ID`, `MSPROLTD_WAKE_REASON`, `MSPROLTD_WAKE_COMMENT_ID`, `MSPROLTD_APPROVAL_ID`, `MSPROLTD_APPROVAL_STATUS`, `MSPROLTD_LINKED_ISSUE_IDS`), user env overrides, and auth token
...
| `MSPROLTD_API_URL` | Server's own URL |
| `MSPROLTD_RUN_ID` | Current run id |
| `MSPROLTD_TASK_ID` | `context.taskId` or `context.issueId` |
| `MSPROLTD_WAKE_REASON` | `context.wakeReason` |
| `MSPROLTD_WAKE_COMMENT_ID` | `context.wakeCommentId` or `context.commentId` |
| `MSPROLTD_APPROVAL_ID` | `context.approvalId` |
| `MSPROLTD_APPROVAL_STATUS` | `context.approvalStatus` |
| `MSPROLTD_LINKED_ISSUE_IDS` | `context.issueIds` (comma-separated) |
| `MSPROLTD_API_KEY` | `authToken` (if no explicit key in config) |
...
Pretty-prints stdout lines for `msproltdai run --watch`. Use `picocolors` for coloring.
...
```ts
import { execute as myExecute, sessionCodec as mySessionCodec } from "@msproltd/adapter-my-agent/server";
import { agentConfigurationDoc as myDoc, models as myModels } from "@msproltd/adapter-my-agent";
...
const myAgentAdapter: ServerAdapterModule = {
  ...
  models: myModels,
  supportsLocalAgentJwt: true,  // true if agent can use MSProLtd API
...
import { parseMyAgentStdoutLine } from "@msproltd/adapter-my-agent/ui";
import { MyAgentConfigFields } from "./config-fields";
import { buildMyAgentConfig } from "@msproltd/adapter-my-agent/ui";
...
```ts
import { printMyAgentStreamEvent } from "@msproltd/adapter-my-agent/cli";
...
- **Unknown session retry**: if resume fails with a "session not found" error, retry with a fresh session and return `clearSession: true` so MSProLtd wipes the stale session
...
Import from `@msproltd/adapter-utils/server-utils`:
...
| `buildPaperclipEnv(agent)` | Standard `MSPROLTD_*` env vars |
...
- Package name: `@msproltd/adapter-<kebab-name>`
...
- Default prompt: `"You are agent {{agent.id}} ({{agent.name}}). Continue your MSProLtd work."`
...
### MSProLtd Skills Injection

MSProLtd ships shared skills (in the repo's top-level `skills/` directory) that agents need at runtime — things like the `mspro-ltd` API skill and the `mspro-ltd-create-agent` workflow skill. Each adapter is responsible for making these skills discoverable by its agent runtime **without polluting the agent's working directory**.

**The constraint:** never copy or symlink skills into the agent's `cwd`. The cwd is the user's project checkout — writing `.claude/skills/` or any other files into it would contaminate the repo with MSProLtd internals, break git status, and potentially leak into commits.

**The pattern:** create a clean, isolated location for skills and tell the agent runtime to look there.

**How claude-local does it:**

1. At execution time, create a fresh tmpdir: `mkdtemp("mspro-ltd-skills-")`
...
async function buildSkillsDir(): Promise<string> {
  const tmp = await fs.mkdtemp(path.join(os.tmpdir(), "mspro-ltd-skills-"));
  const target = path.join(tmp, ".claude", "skills");
  await fs.mkdir(target, { recursive: true });
  const entries = await fs.readdir(MSPROLTD_SKILLS_DIR, { withFileTypes: true });
  for (const entry of entries) {
    if (entry.isDirectory()) {
      await fs.symlink(
        path.join(MSPROLTD_SKILLS_DIR, entry.name),
...
Codex has a global personal skills directory (`$CODEX_HOME/skills` or `~/.codex/skills`). The adapter symlinks MSProLtd skills there if they don't already exist. This is acceptable because it's the agent tool's own config directory, not the user's project.
...
- `MSPROLTD_API_KEY` is injected by the server into the process environment, not the prompt
...
- Prefer minimal allowlists over open internet access. An agent that only needs to call the MSProLtd API and GitHub should not have access to arbitrary hosts.
