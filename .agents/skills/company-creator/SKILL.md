...
description: >
  ...
  is already running in MSProLtd.
---
...
└── .mspro-ltd.yaml            (MSProLtd vendor extension)
```
...
- Agent instructions go in the AGENTS.md body, not in .mspro-ltd.yaml
...
- For companies generated from a repo, add a references footer at the bottom of COMPANY.md body:
  `Generated from [repo-name](repo-url) with the company-creator skill from [MSProLtd](https://github.com/vladimirspecalp-hub/mspro-ltd)`
...
- Citations and references: link to the source repo (if from-repo), link to the Agent Companies spec (https://agentcompanies.io/specification), and link to MSProLtd (https://github.com/vladimirspecalp-hub/mspro-ltd)
- A "Getting Started" section explaining how to import: `msproltdai company import --from <path>`
...
## .mspro-ltd.yaml Guidelines

The `.mspro-ltd.yaml` file is the MSProLtd vendor extension. It configures adapters and env inputs per agent.

### Adapter Rules

**Do not specify an adapter unless the repo or user context warrants it.** If you don't know what adapter the user wants, omit the adapter block entirely — MSProLtd will use its default. Specifying an unknown adapter type causes an import error.

MSProLtd's supported adapter types (these are the ONLY valid values):
...
In this example, only `release-engineer` appears because it needs `GH_TOKEN`. The other agents (ceo, cto, etc.) have no overrides, so they are omitted entirely from `.mspro-ltd.yaml`.
