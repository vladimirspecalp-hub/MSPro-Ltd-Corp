...
description: >
  Handle a GitHub Security Advisory response for MSProLtd, including
...
```
gh api repos/msproltdai/mspro-ltd/security-advisories/{{ghsaId}}
...
```
gh api --method POST \
  repos/msproltdai/mspro-ltd/security-advisories/{{ghsaId}}/forks
...
```
# Clone the private fork somewhere outside ~/mspro-ltd
...
**Do not edit `~/mspro-ltd`** — the dev server is running off the `~/mspro-ltd` master branch and we don't want to touch it. All work happens in the private fork clone.
...
```
gh api --method POST \
  repos/msproltdai/mspro-ltd/security-advisories/{{ghsaId}}/cve
...
```
gh api repos/msproltdai/mspro-ltd/security-advisories/{{ghsaId}} --jq '.credits'
...
```
gh api --method PATCH \
  repos/msproltdai/mspro-ltd/security-advisories/{{ghsaId}} \
  --input - << 'EOF'
{
  "credits": [
    {
...
### Advisory
https://github.com/vladimirspecalp-hub/mspro-ltd/security/advisories/{{ghsaId}}
...
gh release view v{{patchedVersion}} --repo msproltdai/mspro-ltd
...
* The published advisory URL: `https://github.com/vladimirspecalp-hub/mspro-ltd/security/advisories/{{ghsaId}}`
* The release URL
